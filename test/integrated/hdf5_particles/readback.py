...
for step_name in step_names:
    ...
    numpart = int(len(particle_data_bi_all)/8)
    print(numpart)

    print(particle_data_bi_all[:8])
    particle_view_bi_all = particle_data_bi_all.reshape((numpart, 8))
    ...
    for iparticle, particle_name in enumerate(particle_names):
        ...
        if np.allclose(particle_data_bi, particle_data_h5, atol=abs_tol) == False:
...
#            print_max_element = 0
            ...
            for i in range(0, particle_data_h5.shape[0]):
#                if abs(particle_data_bi[i] - particle_data_h5[i]) > abs_tol:
#                if abs(particle_data_bi[i] - particle_data_h5[i]) <= (abs_tol + rel_tol * abs(particle_data_h5[i])):
