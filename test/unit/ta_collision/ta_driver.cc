                                      ...
                                      char ** cmdline_argument )
{
  ...
  auto mean_y = 0.0;
  auto mean_z = 0.0;
  auto sigma_x = vt*sqrt(13.0/11.0);
  auto sigma_y = vt*sqrt(10.0/11.0);
  auto sigma_z = vt*sqrt(10.0/11.0);
...
TEST_CASE( "Check if Weibel gives correct energy (within tol)", "[energy]" )
{
    ...
    const unsigned short t1_mask = 0b00000010;
    const unsigned short t23_mask =0b00001100; 

    // Test just the step range 0-200, and have tight counts
    REQUIRE(
            test_utils::compare_energies(moment_file_name, moment_gold_file_name,
                0.02, t1_mask, test_utils::FIELD_ENUM::Sum, 1, "ta.t1.tight.out", 0, 200)
           );

    // Test the sum of the T2, T3
    REQUIRE(
            test_utils::compare_energies(moment_file_name, moment_gold_file_name,
                0.02, t23_mask, test_utils::FIELD_ENUM::Sum, 1, "ta.t23.tight.out", 0, 200)
           );
