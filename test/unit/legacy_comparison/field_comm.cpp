...
void verify_fields_match(const field_array_t* fa_legacy, const field_array_t* fa_updated, field_var::f_v var) {
  const int nx = fa_legacy->g->nx;
  const int ny = fa_legacy->g->ny;
  const int nz = fa_legacy->g->nz;

  for(int i=0; i<nx+2; i++) {
    for(int j=0; j<ny+2; j++) {
      for(int k=0; k<nz+2; k++) {
        const int voxel = VOXEL(i,j,k,nx,ny,nz);
        REQUIRE(fa_legacy->k_f_h(voxel, var) == fa_updated->k_f_h(voxel, var));
      }
    }
  }
}
  ...
  for(size_t i=0; i<nx+2; i++) {
    for(size_t j=0; j<ny+2; j++) {
      for(size_t k=0; k<nz+2; k++) {
        ...
        fa_legacy->f[cell].cbx = static_cast<float>((rank+1)*(cell+3));
        fa_legacy->f[cell].cby = static_cast<float>((rank+1)*(cell+4));
        fa_legacy->f[cell].cbz = static_cast<float>((rank+1)*(cell+5));
        fa_legacy->f[cell].div_b_err = static_cast<float>((rank+1)*(cell+6));
        fa_legacy->f[cell].tcax = static_cast<float>((rank+1)*(cell+7));
        fa_legacy->f[cell].tcay = static_cast<float>((rank+1)*(cell+8));
        fa_legacy->f[cell].tcaz = static_cast<float>((rank+1)*(cell+9));
        fa_legacy->f[cell].rhof = static_cast<float>((rank+1)*(cell+10));
        fa_legacy->f[cell].rhob = static_cast<float>((rank+1)*(cell+11));
        ...
        fa_updated->f[cell].cbx = static_cast<float>((rank+1)*(cell+3));
        fa_updated->f[cell].cby = static_cast<float>((rank+1)*(cell+4));
        fa_updated->f[cell].cbz = static_cast<float>((rank+1)*(cell+5));
        fa_updated->f[cell].div_b_err = static_cast<float>((rank+1)*(cell+6));
        fa_updated->f[cell].tcax = static_cast<float>((rank+1)*(cell+7));
        fa_updated->f[cell].tcay = static_cast<float>((rank+1)*(cell+8));
        fa_updated->f[cell].tcaz = static_cast<float>((rank+1)*(cell+9));
        fa_updated->f[cell].rhof = static_cast<float>((rank+1)*(cell+10));
        fa_updated->f[cell].rhob = static_cast<float>((rank+1)*(cell+11));
      }
    }
  }
