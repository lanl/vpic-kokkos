 ...
 */
std::pair<bool, double> compare_error(double A, double B, double relative_tolerance, double absolute_tolerance)
{
    ...
    if( err < relative_tolerance* std::abs(B) || err < absolute_tolerance){
	within_tol = true;
    }else {
	within_tol = false;
    }
    ...
    try {
        ...
        else // Performan test
        {
            ...
            while ( getline(f1,line1) )
            {
                ...
                int agg_count = 0;
                while (getline(linestream1, item1, ' ')) //note separated by one space only!
                {
		    ...
		    // std::cout<<item1<<"\n"<<item2<<std::endl;
                    // Use this field
                    //std::cout << "this_line " << total_line_token_count << " mask " << field_mask << std::endl;
		    ...
		    // std::cout<<this_line_token_mask <<","<< field_mask<<std::endl;
		    // printf("this_line_token_mask & field_mask=%d\n",this_line_token_mask & field_mask);
			    ...
			    // std::cout << "sum b " << sum_B << " += " << B << std::endl;
                            ...
                            if (agg_count == agg_total) { // final_aggregation
                                returned_err = compare_error(sum_A, sum_B, relative_tolerance, absolute_tolerance);
                                write_this_err_ouput = true;
                            }
                        }
                        else // We can just compare this val
                        {
                            returned_err = compare_error(A, B, relative_tolerance, absolute_tolerance);
                        }
        ...
        std::cout << "Max found err was " << max_err << " on line " << max_err_line << " (Threshold: relative=" <<
            relative_tolerance << ", absolute ="<< absolute_tolerance<<  std::endl;
