...
std::string energy_tracers_file_name = "./energies_tracers";
                                      ...
                                      char ** cmdline_argument )
{
  ...
  species_t * electron_tracers = define_tracer_species_by_percentage("electron_tracers", electron, TracerType::Copy, 1.0);
  species_t * ion_tracers = define_tracer_species_by_percentage("ion_tracers", ion, TracerType::Move, 1.0);
...
TEST_CASE( "Check if Weibel gives correct energy (within tol)", "[energy]" )
{
    ...
    REQUIRE(
            test_utils::compare_energies(energy_file_name, energy_gold_file_name,
                0.0001, particle_mask, test_utils::FIELD_ENUM::Sum, 1, "Weibel.p.tight.out", 50, 200)
           );
