...
std::string elec_reference_file_name = ELECTRON_REFERENCE_POSITION_FILE;
std::string ion2_reference_file_name = ION2_REFERENCE_POSITION_FILE;
...
std::pair<bool, double> compare_error(double A, double B, double relative_tolerance)
{
    ...
    else if (std::abs(std::min(A,B)) > abs_threshhold)
    {
...
void get_column_indices(std::vector<std::string>& ref_header, int& timestep_idx, int& rank_idx, 
                        int& cell_id_idx, int& posx_idx, int& posy_idx, int& posz_idx)
{
    for(int i=0; i<ref_header.size(); i++) 
    {
      if(ref_header[i].compare("Timestep") == 0)
        timestep_idx = i;
      if(ref_header[i].compare("rank") == 0)
        rank_idx = i;
      if(ref_header[i].compare("cell_id") == 0)
        cell_id_idx = i;
      if(ref_header[i].compare("posx") == 0)
        posx_idx = i;
      if(ref_header[i].compare("posy") == 0)
        posy_idx = i;
      if(ref_header[i].compare("posz") == 0)
        posz_idx = i;
    }
    if(timestep_idx == -1)
      printf("Invalid index for timestep\n");
    if(rank_idx == -1)
      printf("Invalid index for rank\n");
    if(cell_id_idx == -1)
      printf("Invalid index for cell_id\n");
    if(posx_idx == -1)
      printf("Invalid index for posx\n");
    if(posy_idx == -1)
      printf("Invalid index for posy\n");
    if(posz_idx == -1)
      printf("Invalid index for posz\n");
}
...
)
{
    ...
    try {
        ...
        double max_err = 0.0;
        int max_err_line = -1;
        std::tuple<int,int,int64_t> max_err_key;
        std::vector<double> max_err_val;
        ...
        else // Performan test
        {
            ...
            std::map< std::tuple<int,int,int64_t>, std::vector< std::vector<double> > > ref_data;
            std::string header;
            getline(f1, header);
            getline(f2, header);
            std::vector<std::string> ref_header = split(header, ',');
            int timestep_idx = -1, rank_idx = -1, cell_id_idx = -1, posx_idx = -1, posy_idx = -1, posz_idx = -1;
            get_column_indices(ref_header, timestep_idx, rank_idx, cell_id_idx, posx_idx, posy_idx, posz_idx);
 
            std::cout << "Done getting indices for columns\n";

            while(getline(f2, line2)) 
            {
              std::vector<std::string> split_line = split(line2, ',');
              std::tuple<int,int,int64_t> key(std::stoi(split_line[timestep_idx]), std::stoi(split_line[rank_idx]), std::stoll(split_line[cell_id_idx]));
              std::vector<double> value;
              value.push_back(std::stod(split_line[posx_idx]));
              value.push_back(std::stod(split_line[posy_idx]));
              value.push_back(std::stod(split_line[posz_idx]));
              auto iter = ref_data.find(key);
              if(iter == ref_data.end()) 
              {
              ...
              {
                auto& entry = ref_data[key];
                entry.push_back(value);
              }
            }

            std::cout << "Done loading reference data into map\n";

            int matches = 0;
            int counter = 1;
            while(getline(f1, line1))
            {
              std::vector<std::string> split_line = split(line1, ',');
              std::tuple<int,int,int64_t> key(std::stoi(split_line[timestep_idx]), std::stoi(split_line[rank_idx]), std::stoll(split_line[cell_id_idx]));
              double pos[3];
              pos[0] = std::stod(split_line[posx_idx]);
              pos[1] = std::stod(split_line[posy_idx]);
              pos[2] = std::stod(split_line[posz_idx]);
              auto iter = ref_data.find(key);
              if(iter != ref_data.end()) 
              {
                bool found = false;
                int match_idx = 0;
                double match_err = std::numeric_limits<double>::max();
                std::vector<double> line_err_val;
                for(int i=0; i<iter->second.size(); i++)
                {
                  double line_err = 0.0;
                  std::pair<bool, double> xerr = compare_error(pos[0], iter->second[i][0], relative_tolerance);
                  std::pair<bool, double> yerr = compare_error(pos[1], iter->second[i][1], relative_tolerance);
                  std::pair<bool, double> zerr = compare_error(pos[2], iter->second[i][2], relative_tolerance);
                  if(xerr.second > line_err)
                    line_err = xerr.second;
                  if(yerr.second > line_err)
                    line_err = yerr.second;
                  if(zerr.second > line_err)
                    line_err = zerr.second;
                  bool match_line = xerr.first && yerr.first && zerr.first;
                  if(match_line) {
                    found = true;
                  }
                  if(line_err < match_err) {
                    match_err = line_err;
                    match_idx = i;
                    line_err_val = iter->second[i];
                  }
                }
                // Track max absolute error
                if (match_err > max_err)
                {
                    max_err = match_err;
                    max_err_line = counter;
                    max_err_key = key;
                    max_err_val = line_err_val;
                }
                if(found)
                {
                  matches += 1;
            ...
            printf("Number of matches: %d / %d\n", matches, counter-1);
            match = matches == counter-1;
        ...
        std::cout << "Rank " << rank << " Max found err was " << max_err*100 << "% on line " << max_err_line << " (Threshold: " <<
            relative_tolerance*100 << "%)" << std::endl;
        if(max_err > 0) 
          printf("\t(%d,%d,%ld): (%e,%e,%e)\n", std::get<0>(max_err_key), std::get<1>(max_err_key), std::get<2>(max_err_key), max_err_val[0], max_err_val[1], max_err_val[2]);
...
void dump_particle_pos( const char *sp_name,
  ...
  if( ftag ) {
      snprintf( fname, 256, "%s.%li.%i", fbase, (long)step, rank );
  }
  else {
      snprintf( fname, 256, "%s.%i", fbase, rank );
  }
  ...
  std::string header_str = std::string("Timestep,rank,cell_id,dx,dy,dz,ux,uy,uz,w,posx,posy,posz");
...
#define _nxg (sp->g->nx + 2)
#define _nyg (sp->g->ny + 2)
#define _nzg (sp->g->nz + 2)
#define i0 (ii%_nxg)
#define j0 ((ii/_nxg)%_nyg)
#define k0 (ii/(_nxg*_nyg))
#define tracer_x ((i0 + (dx0-1)*0.5) * sp->g->dx + sp->g->x0)
#define tracer_y ((j0 + (dy0-1)*0.5) * sp->g->dy + sp->g->y0)
#define tracer_z ((k0 + (dz0-1)*0.5) * sp->g->dz + sp->g->z0)
  // Write tracer data
...
#define m_e_SI (9.1093837015e-31) /* kg */
  ...
  double laser_intensity_SI    = 3e26;      // units of W/m^2
...
//  double nppc = 60;  // Average number of macro particles/cell of each species
  double nppc = 5;  // Average number of macro particles/cell of each species
...
#define mp_me 1836.15267343
  ...
  double pulse_FWHM = 150e-15 / time_to_SI; // time in code units
...
//???????????????????????????????????????????????????????????????????????
  ...
  species_t * electron_small = define_species("electron_small", -1.*e_c, m_e_c,
          max_local_np_e, max_local_nm_e, 20, 0);
  species_t * electron = define_species("electron", -1.*e_c, m_e_c,
          max_local_np_e, max_local_nm_e, 20, 0);
//          max_local_np_e, max_local_nm_e, 0, 0);
...
# define iv_region ( x<global->xmin + dx*iv_thick || x>global->xmax-dx*iv_thick\
  ...
  if ( load_particles!=0 ) {
    ...
    printf("Rank %d Grid: (%f,%f,%f) (%f,%f,%f)\n", rank(), xmin, ymin, zmin, xmax, ymax, zmax);
...
//                             normal( rng(0), 0, uthi_I2 ), qi_I2 );
    ...
    repeat( (800)/(topology_x*topology_y*topology_z) ) {
      double x = uniform( rng(0), xmin, xmax );
      double y = uniform( rng(0), ymin, ymax );   
      double z = uniform( rng(0), zmin, zmax );
      if ( iv_region ) continue;   // Particle fell in iv_region.  Don't load.
      ...
      if ( uniform( rng(0), 0, 1 ) < slab(x*length_to_SI, y*length_to_SI,
            ...
            // third to last arg is "weight," a positive number
            //std::cout<< " injecting electron " << std::endl;
            inject_particle( electron_small, x, y, z,
                           normal( rng(0), 0, px_e_norm ),
                           normal( rng(0), 0, px_e_norm ),
                           normal( rng(0), 0, px_e_norm ), fabs(qe), 0, 0 );
          if ( mobile_ions ) {
              inject_particle( ion_I2_small, x, y, z,
                               normal( rng(0), 0, px_I2_norm ),
                               normal( rng(0), 0, px_I2_norm ),
                               normal( rng(0), 0, px_I2_norm ), fabs(qi_I2)/Z_I2,
                               0, 0 );
    ...
    species_t* ion_I2_tracers   = define_tracer_species_with_n("I2_tracers", ion_I2_small, TracerType::Copy, ion_I2_small->np);
    species_t* electron_tracers = define_tracer_species_by_percentage("electron_tracers", electron_small, TracerType::Move, 100);
    printf("Electrons: small vs tracers: %d vs %d\n", electron_small->np, electron_tracers->np);
    printf("I2:        small vs tracers: %d vs %d\n", ion_I2_small->np, ion_I2_tracers->np);
...
# define should_dump(x) \
  ...
  dump_particle_pos( "electron_tracers", species_list, tracers_list, "electron_dump", step() != 0, 0, step(), rank() );
  dump_particle_pos( "I2_tracers", species_list, tracers_list, "I2_dump", step() != 0, 0, step(), rank() );
//  dump_particles_csv( "electron_small", DumpVar::GlobalPos,
//                      "electron_dump", step() != 0, 0);
//  dump_particles_csv( "I2_small", DumpVar::GlobalPos,
//                      "I2_dump", step() != 0, 0);
//  dump_particles_csv( "electron", DumpVar::GlobalPos,
//                      "electron_dump", step() != 0, 0);
//  dump_particles_csv( "I2", DumpVar::GlobalPos,
//                      "I2_dump", step() != 0, 0);
//  dump_particles_csv( "electron_tracers", DumpVar::GlobalPos,
//                      "electron_dump", step() != 0, 0);
//  dump_particles_csv( "I2_tracers", DumpVar::GlobalPos,
//                      "I2_dump", step() != 0, 0);
#endif

  if(step() == num_step) {
    bool elec_res = compare_position(std::string("electron_dump.")+std::to_string(rank()),
                                     elec_reference_file_name, 1.0, rank());
    bool ion2_res = compare_position(std::string("I2_dump.")+std::to_string(rank()),
                                     ion2_reference_file_name, 1.0, rank());
    if(elec_res == false) {
      printf("Aborting: Electron tracers did not match\n");
      abort(1);
    } else if(ion2_res == false) {
      printf("Aborting: Ion2 tracers did not match\n");
      abort(2);
    }
  }
