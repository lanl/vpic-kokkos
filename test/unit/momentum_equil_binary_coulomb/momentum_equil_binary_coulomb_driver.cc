                                           ...
                                           char ** cmdline_argument )
{
  ...
  double cvar0 = (ion->q * ion->q) * (beam->q *  beam->q) * lnL_8pi;

  CCModel ccm;
  define_collision_op(binary_coulomb("ta", ion, beam, cvar0, ccm, ncoll_coulomb, var_wt));
  ...
  double tau_ib = 1.0 / (std::sqrt(2) * cvar0);
