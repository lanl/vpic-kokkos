#ifndef __MATMUL_KERNEL_HPP__
#define __MATMUL_KERNEL_HPP__
...
// Reference GEMM from Kokkos Kernels
template <class ExecSpace, class ViewTypeA, class ViewTypeB, class ViewTypeC, int block A0, int blockA1, int block B1, int TransposeA, int TransposeB>
struct GEMMRef {
  ViewTypeA A;
  ViewTypeB B;
  ViewTypeC C;
  ...
  GEMMRef(const ScalarA& alpha_, const ViewTypeA& A_, const ViewTypeB& B_, const ScalarC& beta_, const ViewTypeC& C_)
      : A(A_),
        B(B_),
        C(C_),
