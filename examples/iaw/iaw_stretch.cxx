...
double xmin = grid->x0, xmax = grid->x1;  // Computational space bounds
double ymin = grid->y0, ymax = grid->y1;
double zmin = grid->z0, zmax = grid->z1;

// Stretching parameters (should match init_stretched_cartesian_grid call)
double beta_x = 2.0;  // Match your grid initialization
double beta_y = 0.0;
double beta_z = 0.0;

// ALTERNATIVE: More efficient approach
// Generate in physical space, convert to computational space

repeat( Ni ) {
  double x_phys, y_phys, z_phys;
  double r;
  
  // Rejection method in physical space
  do {
    x_phys = uniform( rng(0), -Lx/2, Lx/2 );
    r = uniform(rng(0), 0, 1.0 + pert);
  } while( r > (1.0 + pert * sin(kx * x_phys)) );
  ...
  // Check if this rank owns this computational coordinate
  if (xi_comp >= xmin && xi_comp < xmax) {
    y_phys = uniform( rng(0), -Ly/2, Ly/2 );
    z_phys = uniform( rng(0), -Lz/2, Lz/2 );
    
    // For uniform grids in y,z, physical = computational
    double y_comp = y_phys;
    double z_comp = z_phys;
    
    double ux = normal( rng(0), 0, vthi );
    double uy = normal( rng(0), 0, vthi );
    double uz = normal( rng(0), 0, vthi );
    
    inject_particle( ion, xi_comp, y_comp, z_comp, ux, uy, uz, qi, 0, 0 );
  }
}
