...
! creat view, open MPI file, etc
  ...
  fnames(29) = 'data/ui3z'
  fnames(30) = 'data/ni3'
  fnames(31) = 'data/pi3-xx'
  fnames(32) = 'data/pi3-yy'
  fnames(33) = 'data/pi3-zz'
  fnames(34) = 'data/pi3-yz'
  fnames(35) = 'data/pi3-xz'
  fnames(36) = 'data/pi3-xy'
     ...
     do dom_x = ht%start_x,ht%stop_x
        do dom_y = ht%start_y,ht%stop_y
           do dom_z = ht%start_z, ht%stop_z
              ...
              write(fname,"(A,I0,A,I0,A,I0)")"hydro/T.",tindex,"/Thydro.",tindex,".",n-1        
              if (check) then 
                 open(unit=10,file=trim(fname),status='unknown',form='unformatted',access='stream')
              else
                 print *,"Can't find file:",fname
