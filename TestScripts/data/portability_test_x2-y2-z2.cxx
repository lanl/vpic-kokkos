//========================================================================
// LPI test deck based on SBS single speckle simulations from Albright, Yin,
// Bowers, and Bergen, Physics of Plasmas, 2016.  There is no Maxwellian
// reinjection, and there are two laser pulses to make it less like a uniform
// cold plasma sooner.
//
// Modified from a short pulse deck used by Scott V. Luedtke.
//
// Scott V. Luedtke, XCP-6, 1/7/21
//========================================================================

//??????????????????????????????????????????????????????????????????????

#define ENABLE_OUTPUT 0
#define ENABLE_RIGHT_LASER 1

#include <ctime>

// This is probably unnecessary on modern parallel file systems, e.g., lustre
#define NUM_TURNSTILES 30000000  
...
#define m_e_SI (9.1093837015e-31) /* kg */
  ...
  // Physical parameters
  double n_e_over_n_crit    = 0.05;       // n_e/n_crit in solid slab
  double laser_intensity_SI    = 2.5e20;      // units of W/m^2
  double lambda_SI  = 527e-9; // 800 nm laser
  double w0_SI = 4e-6; // Beam waist

  // Full scale topology is 60*15*15, and don't forget to turn on the right
  // laser.  Changing the topology will automatically adjust box size and
  // resolution for strong scaling, but beware the iv_region!
  int topology_x = 2;
  int topology_y = 2;
  int topology_z = 2;

  double Lx_SI         = (80e-6/60.)*2.; // Simulation box size
  double Ly_SI         = (20e-6/15.)*2.;//w0_SI*sqrt(M_PI/2.);  // 3DCHANGE
  double Lz_SI         = (20e-6/15.)*2.;
  // Stop when the 2 pulses collide
  double t_stop = (80e-6/2.)/c_SI / time_to_SI; // Simulation run time

  double T_e = 600. * e_SI; // Technically, this is k_B*T.  e_SI is eV to J.
  double T_i = 150. * e_SI;
  float dfrac               = 0.0; // fraction of charge density for n_Al12/ne

  double dist_to_focus = Lx_SI/2.;// For this run, put the origin at the focus
  ...
  // Adjust nppc to account for the iv_region
  // What we want is for the most-loaded rank to have particles equal to
  // 80*(total cells that would fill the domain volume if total resolution is
  // 200^3) including the iv_region (excluding ghost cells).  On 8 ranks with a
  // 2x2x2 configuration, this is pretty easy: Each rank has 3 faces that
  // neighbor other ranks and 3 that are on the boundary, and so have to fit
  // 80*100^3 particles in (100-2)^3 cells, meaning we adjust nppc like this:
  double nppc = 80.*double(pow(100,3))/(98.*98.*98.);
  // When all of the topologies are >= 3, there is at least one center rank
  // with no boundary, and therefore no iv_region, so if the overall resolution
  // stays at 200^3, we can simply set nppc like we would naively expect:
  // double nppc = 80;

  // When we're on Rome and have 8*64 ranks, the math works out really well.
  // 8*64=8x8x8, and 8 divides 200 so decompose 8x8x8 and set nppc=80.

  // Cascade Lake probably won't be as fun.  We've got 28*8=224 ranks.  Wait,
  // do we even want to run one rank per core?  Is that best in the Kokkos
  // version?  Bob guesses yes, so we'd better at least test it.  So, we've got
  // 224 ranks, and 7 is a factor of 224, and last I checked, that doesn't
  // divide 200.  Now, we could "cheat" and not use all the cores, but that's
  // not the most fair.  Better is to tweak the resolution and adjust nppc so
  // there are the same number of particles as in a 200^3 run.  (I can't
  // imagine this affecting the physics significantly.  I also fixed the number
  // of steps to 5000, so we don't have to worry about the CFL condition
  // changing how long we run for.)  The closest multiple of 7 to 200 is 203,
  // so lets make ny=203.  (ny is my least favorite.)  It's easy enough to
  // figure out the rest of the topology in this case.  Let's pick 4x7x8 for
  // 200x203x200.  Now before we had 80*200^3 particles, and we want to keep
  // that total.  Easy:
  //double nppc = 80.*pow(200.,3)/(200.*203.*200);
  // And we don't need to adjust this further because all the topologies are >=
  // 3

  // Probably don't need restarts for this test
  double quota = 1e10;             // Run quota in hours.  
...
#define mp_me 1836.15267343
  ...
  // Laser parameters
  int pulse_shape=4;                // Ad-hoc triangle
  double pulse_FWHM = 300e-15 / time_to_SI; // time in code units
  // How far in front of the boundary should the peak start?
  double pulse_start = t_stop/6.;//5e-12 / time_to_SI; // time in code units
  double lambda    = lambda_SI / length_to_SI;  // Wavelength
  double xfocus    = dist_to_focus / length_to_SI; // Distance from boundary to
                                                   // focus
  double f_number  = 6; // Not used!    // f number of beam
...
// if plane wave:
  ...
  int field_interval    = 500;//int(5./omega_L_SI / time_to_SI / dt);
  ...
  // Probably don't want to restart this test
  int restart_interval = 0;
  int quota_check_interval = 0;
...
//???????????????????????????????????????????????????????????????????????
  ...
  int nspec = 2;
  // Sorting uses lots of memory
  species_t * electron = define_species("electron", -1.*e_c, m_e_c,
          max_local_np_e, max_local_nm_e, 25, 0);
  ...
  sim_log("Setting up ions. ");
    ...
    if ( I2_present  ) {
        ion_I2 = define_species("I2", Z_I2*e_c, m_I2_c,
            max_local_np_i2, max_local_nm_i2, 50, 0);
...
//"impermeable_vacuum_xr" );
  ...
  if ( load_particles!=0 ) {
    ...
    repeat( (Ne)/(topology_x*topology_y*topology_z) ) {
      ...
      if ( uniform( rng(0), 0, 1 ) < 99) { //Uniform plasma everywhere
...
#define LEFT_BOUNDARY (grid->x0<(float(global->xmin)+0.5*grid->dx))
#define RIGHT_BOUNDARY (grid->x1>(float(global->xmax)-0.5*grid->dx))

  if ( LEFT_BOUNDARY || RIGHT_BOUNDARY ) { // Node is on left boundary
    ...
    if (global->pulse_shape==3){
        if (t>global->pulse_start) pulse_shape_factor=1.;
        else{
    ...
    if ( global->pulse_shape==4 ) {
      pulse_shape_factor =( t/(num_step*grid->dt) );
    }
#endif 
...
// Kokkos Port
    ...
    if RIGHT_BOUNDARY {
        Kokkos::MDRangePolicy<Kokkos::Rank<2>> right_edge({1, 1}, {nz+2, ny+1});
        Kokkos::parallel_for("Field injection", right_edge, KOKKOS_LAMBDA(const int iz, const int iy) {
            ...
            auto PHASE=( omega_0*t + h*R2/(width*width) );
            auto MASK =( R2<=pow(mask*width,2) ? 1 : 0 );
            kfield(nx+1+sy*iy+sz*iz, field_var::ey) += (prefactor * -1*cos(PHASE) * exp(-R2/(width*width)) * MASK * pulse_shape_factor);
        });
    }
#endif
