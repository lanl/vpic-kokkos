...
begin_initialization {
  ...
  double n0 = 1.0/2;    //(will use two ion species) Density 
  ...
  double nppc  = 150000; //0;    // Average number of macro particle per cell per species 
  ...
  species_t *i2n = define_species("i2n", ec, mi, nmax, nmovers, sort_interval, sort_method);
...
// In general, regions are specied as logical equations (i.e. x>0 && x+y<2) 
 ...
 repeat( Ni ) {
    ...
    if (x>=xmin && x<= xmax) {
      ...
      uy = normal( rng(0), 0, vthi );
      uz = normal( rng(0), 0, vthi );
      inject_particle( i2n, x, y, z, ux, uy, uz, qi, 0, 0 );
    }
 }
  ...
  //set collisions
  // C_coll the normalization coefficient in front of collision operator
  double c_coll = 0.1; 
  
  auto M_ln_Lamda = 10;
  double cvar0 = c_coll*M_ln_Lamda/(8.0*M_PI);       //in SI: (e^4*n0*Lambda)/(8*pi*eps0^2*m_e^2*c^3)
  // --> internal operator multiplies by dt!

  int ncoll = 1;  // How frequently to do collisions
  
  define_collision_op(takizuka_abe("ta_coll", ion, i2n, cvar0, ncoll));
  define_collision_op(takizuka_abe("ta_coll", ion, ion, cvar0, ncoll));
  define_collision_op(takizuka_abe("ta_coll", i2n, i2n, cvar0, ncoll));
  ion->last_indexed = -1;
  i2n->last_indexed = -1;
