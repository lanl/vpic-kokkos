...
//int    psum_integration_offset = int(iv_thick/2);
  ...
  float box_size_x = 100.0 * 1e-4; //158.0 * 1e-4;   // Microns
  float box_size_z = 1.0 *  1e-4; //14.5 *  1e-4;   // Microns (ignored if 1d or 2d in plane)
...
# endif
  ...
  sim_log("* Time step, max time, nsteps =  "<<dt<<" "<<t_stop<<" "<< num_step);
