...
//  double f_He             = 0.5;        // Ratio of number density of He to total ion density
  double f_He             = 0.3;        // Ratio of number density of He to total ion density
...
//  int ele_sort_freq       = 2000; 
//  int ion_sort_freq       = 2000; 
...
//      ion_H  = define_species("H",  Z_H,  mime_H,  qi_H, max_local_np, max_local_nm, ion_sort_freq, 0);
      ion_H  = define_species("H",  Z_H,  mime_H,  max_local_np*f_H, max_local_nm*f_H, ion_sort_freq, 0);
    }
    if ( He_present ) {
      sim_log("- Creating He species.");
      ion_He = define_species("He", Z_He, mime_He, max_local_np*f_He, max_local_nm*f_He, ion_sort_freq, 0);
    }
  }
...
# define DENSITY( Z, X ) \
  ...
  if ( load_particles ) {
    ...
    repeat( f_H*Ne * (NE_NCR_MAX / NE_NCR_MEAN) / (topology_x*topology_y*topology_z) ) {
      double x = uniform( rng(0), xmin, xmax );
      double y = uniform( rng(0), ymin, ymax );
      double z = uniform( rng(0), zmin, zmax );
      if ( iv_region ) continue;           // Particle fell in iv_region.  Don't load.
    ...
    repeat( f_He * Ne * (NE_NCR_MAX / NE_NCR_MEAN) / (topology_x*topology_y*topology_z) ) {
      double x = uniform( rng(0), xmin, xmax );
      double y = uniform( rng(0), ymin, ymax );
      double z = uniform( rng(0), zmin, zmax );
      if ( iv_region ) continue;           // Particle fell in iv_region.  Don't load.
