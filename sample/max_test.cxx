...
#define m_e_SI (9.1093837015e-31) /* kg */
  ...
  // Physical parameters
  double n_e_over_n_crit    = 0.5;       // n_e/n_crit in solid slab
  ...
  double dist_to_focus = 1e-6;// For this run, put the origin at the focus
...
//???????????????????????????????????????????????????????????????????????
  ...
  electron->ut_perp = px_e_norm*0.05;
  electron->ut_para = px_e_norm*0.05;
  ...
  sim_log("Setting up ions. ");
    ...
    if ( I2_present  ) {
        ...
        ion_I2->ut_perp = px_I2_norm*0.05;
        ion_I2->ut_para = px_I2_norm*0.05;
...
# define iv_region ( x<global->xmin + dx*iv_thick || x>global->xmax-dx*iv_thick\
  ...
  //absorb_particles);
  set_region_bc( iv_region, Maxwellian_reflux, Maxwellian_reflux,
  Maxwellian_reflux);
  //set_region_bc(iv_region, -3, -3, -3);
  ...
  if ( load_particles!=0 ) {
    ...
    repeat( (Ne)/(topology_x*topology_y*topology_z) ) {
      ...
      //            z*length_to_SI, global->xmin*length_to_SI, 3e-6,
