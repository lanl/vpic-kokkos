// Dipole + solar wind.  
//Mercury has offset dipole moment
...
begin_initialization {
  ...
  double beta_b  = .05; //1.0;  //ion background beta based on B=1
  double R_P     = 150.0;  //planet radius in di
  double RM_RP   = 1.;  //2D magnetopuase standoff/planet radius
  double bximf   = 0.933;
  double byimf   = 0;
  double bzimf   = -0.21;//sin(-45.*M_PI/180);
  double vdx     = 2.;
  double vdy     = 0;
  double vdz     = 0;
  
  double taui   =  800; //ion cyclotron times to run

  // Numerical parameters
  double Lx        = 1600*di;  //1620*di;   // How big should the box be in the x direction
  double Ly        = 1*di;     // How big should the box be in the y direction
  double Lz        = 3200*di; //1600*di;  // How big should the box be in the z direction
  double nx        = 800;      // Global resolution in the x direction
  double ny        = 1;        // Global resolution in the y direction
  double nz        = 800;     // Global resolution in the z direction
  double nppc      = 100; //50;       // Average number of macro particles per cell per species
  ...
  double x_d = x_P;
  double y_d = y_P;
  double z_d = z_P;// +0.2*R_P; //Mercury shift to the north
...
# define RANK_TO_INDEX(rank,ix,iy,iz) BEGIN_PRIMITIVE {                   \
  ...
  status_interval      = int(4./dt);  //4:2000
  ...
  global->restart_interval   = status_interval*1.0; // Do not dump
...
// Parameters for the open boundary model
  ...
  //grid->den = 1.0;
  grid->eta = 1.0e-3;
  grid->hypereta=1.0e-2;

  //if(top | bottom) grid->hypereta=0.2;
  //if(left |right)  grid->hypereta=1.0;

 grid->nsub = 1.0; //number of field subcycles
  grid->nsmb=500;
  grid->nsm=0;
  ...
  material_t * layer = define_material("layer", 1.,1,1.,
					         1.,1.,1.,
					         0.,0.,0.);

      material_t * layer2 = define_material("layer2", 1.,50.,1.,
					         1.,1.,1.,
					         0.,0.,0.);
...
# define INSIDE_PLANET (R2P < R_P*R_P)
  ...
  set_region_eta_multipliers(INSIDE_PLANET, 0,0,0);


# define INSIDE_LAYER ( ((x*x + z*z)>(R_P)*(R_P)) && ((x*x + z*z)<((R_P+8*hx)*(R_P+8*hz))) )

  //set_region_material(INSIDE_LAYER, layer, layer);
   // set_region_eta_multipliers(INSIDE_LAYER, 1,1,1);
    
    
# define LEFT_LAYER (x < -0.45*Lx )

  //set_region_material(INSIDE_LAYER, layer, layer);
  //set_region_eta_multipliers(LEFT_LAYER, 1,1,1);
...
#define MDBZ (MDM*(MZDT/(MXD*MXD+MZDT*MZDT))-MDM*(MZDB/(MXD*MXD+MZDB*MZDB)))
    ...
    set_region_bext( everywhere, DBX+MDBX+bximf, 0+byimf, DBZ+MDBZ+bzimf); // Magnetic field
 		     
   set_region_te( everywhere, kTe);
  ...
  if (global->left) {
    for ( int n=1; n<=nsp; n++ ) {
	  ...
	  double press[9] = {pleft(1,1,n,k,j),pleft(1,2,n,k,j),pleft(1,3,n,k,j),
	    ...
	    inject_particle_r(species, x, y, z, uv[0], uv[1], uv[2], q(n), age, 0 );
...
#define X_LOOP(x) XYZ_LOOP(x,x,1,ny,1,nz)
#define Y_LOOP(x) XYZ_LOOP(1,nx,y,y,1,nz)
#define Z_LOOP(x) XYZ_LOOP(1,nx,1,ny,z,z)
...
k_field_t& k_field = field_array->k_f_d;
  ...
  // LEFT Boundary
  if (global->left) {
		Kokkos::parallel_for("inject_fields: x_face_loop", x_face, KOKKOS_LAMBDA(const int z, const int y) {
                    //k_field(VOXEL(0,y,z,nx,ny,nz), field_var::cbx) *= r;
                    k_field(VOXEL(0,y,z,nx,ny,nz), field_var::cby) *= r;
                    k_field(VOXEL(0,y,z,nx,ny,nz), field_var::cbz) *= r;
                    
                    //k_field(VOXEL(1,y,z,nx,ny,nz), field_var::cbx) *= r;
                    k_field(VOXEL(1,y,z,nx,ny,nz), field_var::cby) *= r;
                    k_field(VOXEL(1,y,z,nx,ny,nz), field_var::cbz) *= r;
	    	});
	}
  ...
  if (global->right) {
		Kokkos::parallel_for("inject_fields: x_face_loop", x_face, KOKKOS_LAMBDA(const int z, const int y) {
                    //k_field(VOXEL(nx+1,y,z,nx,ny,nz), field_var::cbx) *= r;
                    k_field(VOXEL(nx+1,y,z,nx,ny,nz), field_var::cby) *= r;
                    k_field(VOXEL(nx+1,y,z,nx,ny,nz), field_var::cbz) *= r;
                    
                    //k_field(VOXEL(nx,y,z,nx,ny,nz), field_var::cbx) *= r;
                    k_field(VOXEL(nx,y,z,nx,ny,nz), field_var::cby) *= r;
                    k_field(VOXEL(nx,y,z,nx,ny,nz), field_var::cbz) *= r;
  ...
  // TOP Boundary
  if (global->top) {
		Kokkos::parallel_for("inject_fields: z_face_loop", z_face, KOKKOS_LAMBDA(const int y, const int x) {
                    k_field(VOXEL(x,y,nz+1,nx,ny,nz), field_var::cbx) *= r;
                    k_field(VOXEL(x,y,nz+1,nx,ny,nz), field_var::cby) *= r;
                    //k_field(VOXEL(x,y,nz+1,nx,ny,nz), field_var::cbz) *= r;
                   
                    k_field(VOXEL(x,y,nz,nx,ny,nz), field_var::cbx) *= r;
                    k_field(VOXEL(x,y,nz,nx,ny,nz), field_var::cby) *= r;
                    //k_field(VOXEL(x,y,nz,nx,ny,nz), field_var::cbz) *= r;
  ...
  if (global->bottom) {
		Kokkos::parallel_for("inject_fields: z_face_loop", z_face, KOKKOS_LAMBDA(const int y, const int x){
                    k_field(VOXEL(x,y,0,nx,ny,nz), field_var::cbx) *= r;
                    k_field(VOXEL(x,y,0,nx,ny,nz), field_var::cby) *= r;
                    //k_field(VOXEL(x,y,0,nx,ny,nz), field_var::cbz) *= r;
                    
                    k_field(VOXEL(x,y,1,nx,ny,nz), field_var::cbx) *= r;
                    k_field(VOXEL(x,y,1,nx,ny,nz), field_var::cby) *= r;
                    //k_field(VOXEL(x,y,1,nx,ny,nz), field_var::cbz) *= r;
