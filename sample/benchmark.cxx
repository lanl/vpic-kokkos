...
// Bowers, and Bergen, Physics of Plasmas, 2016.  Results from a version of
// this deck are reported in Bird, Tan, Luedtke, Harrell, Taufer, and Albright,
// IEEE Transactions on Parallel and Distributed Systems, 2021.
//
// There are some subtleties around the impermeable vacuum and MPI
// communication that may affect comparisons across platforms or when changing
// the topology.
//
...
#define m_e_SI (9.1093837015e-31) /* kg */
  ...
  double Ly_SI         = (20e-6/15.)*1.;//w0_SI*sqrt(M_PI/2.);  // 3DCHANGE
  double Lz_SI         = (20e-6/15.)*1.;
  // Check num_step below if you want this to be honored.
...
//???????????????????????????????????????????????????????????????????????
  ...
  // As of 12/21, these are not resized in the Kokkos version of VPIC
...
#define LEFT_BOUNDARY (grid->x0<(float(global->xmin)+0.5*grid->dx))
  ...
  if ( LEFT_BOUNDARY ) { // Node is on left boundary
