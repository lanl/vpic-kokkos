...
# define BEGIN_RECV(i,j,k,X,Y,Z) \
  begin_recv_port(i,j,k,( 1 + 15*(n##Y+1)*(n##Z+1) )*sizeof(float),g)

# define BEGIN_SEND(i,j,k,X,Y,Z) BEGIN_PRIMITIVE {      \
    size = ( 1 + 15*(n##Y+1)*(n##Z+1) )*sizeof(float);  \
...
# define ADJUST_HYDRO(i,j,k,X,Y,Z)                              \
  do {                                                          \
	...
	k_h_d(VOXEL(x,y,z,nx,ny,nz), hydro_var::n_p) *= 2;      \
      });                                       \
    }                                           \
  } while(0)
...
#define BEGIN_RECV_KOKKOS(i,j,k,X,Y,Z,recv_buff_h) \
  begin_recv_port_k(i,j,k,(1 + 15*(n##Y+1)*(n##Z+1))*sizeof(float),g,reinterpret_cast<char*>(recv_buff_h.data()));
...
# define BEGIN_SEND_KOKKOS(i,j,k,X,Y,Z,send_buff_d,send_buff_h) BEGIN_PRIMITIVE {      \
    ...
    KOKKOS_LAMBDA(const int x, const int y, const int z) { \
      send_buff_d(1+15*((Z-1)*(n##Y)+(Y-1)) + 0) = k_h_d(VOXEL(x,y,z,nx,ny,nz), hydro_var::jx); \
      send_buff_d(1+15*((Z-1)*(n##Y)+(Y-1)) + 1) = k_h_d(VOXEL(x,y,z,nx,ny,nz), hydro_var::jy); \
      send_buff_d(1+15*((Z-1)*(n##Y)+(Y-1)) + 2) = k_h_d(VOXEL(x,y,z,nx,ny,nz), hydro_var::jz); \
      send_buff_d(1+15*((Z-1)*(n##Y)+(Y-1)) + 3) = k_h_d(VOXEL(x,y,z,nx,ny,nz), hydro_var::rho); \
      send_buff_d(1+15*((Z-1)*(n##Y)+(Y-1)) + 4) = k_h_d(VOXEL(x,y,z,nx,ny,nz), hydro_var::px); \
      send_buff_d(1+15*((Z-1)*(n##Y)+(Y-1)) + 5) = k_h_d(VOXEL(x,y,z,nx,ny,nz), hydro_var::py); \
      send_buff_d(1+15*((Z-1)*(n##Y)+(Y-1)) + 6) = k_h_d(VOXEL(x,y,z,nx,ny,nz), hydro_var::pz); \
      send_buff_d(1+15*((Z-1)*(n##Y)+(Y-1)) + 7) = k_h_d(VOXEL(x,y,z,nx,ny,nz), hydro_var::ke); \
      send_buff_d(1+15*((Z-1)*(n##Y)+(Y-1)) + 8) = k_h_d(VOXEL(x,y,z,nx,ny,nz), hydro_var::txx); \
      send_buff_d(1+15*((Z-1)*(n##Y)+(Y-1)) + 9) = k_h_d(VOXEL(x,y,z,nx,ny,nz), hydro_var::tyy); \
      send_buff_d(1+15*((Z-1)*(n##Y)+(Y-1)) + 10) = k_h_d(VOXEL(x,y,z,nx,ny,nz), hydro_var::tzz); \
      send_buff_d(1+15*((Z-1)*(n##Y)+(Y-1)) + 11) = k_h_d(VOXEL(x,y,z,nx,ny,nz), hydro_var::tyz); \
      send_buff_d(1+15*((Z-1)*(n##Y)+(Y-1)) + 12) = k_h_d(VOXEL(x,y,z,nx,ny,nz), hydro_var::tzx); \
      send_buff_d(1+15*((Z-1)*(n##Y)+(Y-1)) + 13) = k_h_d(VOXEL(x,y,z,nx,ny,nz), hydro_var::txy); \
      send_buff_d(1+15*((Z-1)*(n##Y)+(Y-1)) + 14) = k_h_d(VOXEL(x,y,z,nx,ny,nz), hydro_var::n_p); \
    }); \
...
#define END_SEND_KOKKOS(i,j,k,X,Y,Z) end_send_port_k(i,j,k,g);
  ...
  Kokkos::View<float*> sbuf_pos_d("Send buffer pos", 1+15*(ny+1)*(nz+1));
  Kokkos::View<float*> sbuf_neg_d("Send buffer neg", 1+15*(ny+1)*(nz+1));
  Kokkos::View<float*> rbuf_pos_d("Recv buffer pos", 1+15*(ny+1)*(nz+1));
  Kokkos::View<float*> rbuf_neg_d("Recv buffer neg", 1+15*(ny+1)*(nz+1));
  ...
  Kokkos::resize(sbuf_pos_d, 1+15*(nz+1)*(nx+1));
  Kokkos::resize(sbuf_neg_d, 1+15*(nz+1)*(nx+1));
  Kokkos::resize(rbuf_pos_d, 1+15*(nz+1)*(nx+1));
  Kokkos::resize(rbuf_neg_d, 1+15*(nz+1)*(nx+1));
  ...
  Kokkos::resize(sbuf_pos_d, 1+15*(nx+1)*(ny+1));
  Kokkos::resize(sbuf_neg_d, 1+15*(nx+1)*(ny+1));
  Kokkos::resize(rbuf_pos_d, 1+15*(nx+1)*(ny+1));
  Kokkos::resize(rbuf_neg_d, 1+15*(nx+1)*(ny+1));
  sbuf_pos_h = Kokkos::create_mirror_view(sbuf_pos_d);
  sbuf_neg_h = Kokkos::create_mirror_view(sbuf_neg_d);
  rbuf_pos_h = Kokkos::create_mirror_view(rbuf_pos_d);
  rbuf_neg_h = Kokkos::create_mirror_view(rbuf_neg_d);
