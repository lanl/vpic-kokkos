...
//            Kokkos::atomic_add(&(dst[i]),  src[i]);
    ...
    KOKKOS_INLINE_FUNCTION
    void operator() (const int n, value_type sums) const {
        float w0, w1, w2, w3, w4, w5, w6, w7, dz;
...
#   undef FMA
        ...
        sums[v+sz+sy+1] += w7;
/*
        Kokkos::atomic_add(&(sums[v         ]), w0);
        Kokkos::atomic_add(&(sums[v+1       ]), w1);
        Kokkos::atomic_add(&(sums[v+sy      ]), w2);
        Kokkos::atomic_add(&(sums[v+sy+1    ]), w3);
        Kokkos::atomic_add(&(sums[v+sz      ]), w4);
        Kokkos::atomic_add(&(sums[v+sz+1    ]), w5);
        Kokkos::atomic_add(&(sums[v+sz+sy   ]), w6);
        Kokkos::atomic_add(&(sums[v+sz+sy+1 ]), w7);
*/
    }
    ...
    void final(value_type dst) const {
        for(int i=0; i<value_count; i++) {
            kfield(i, field_var::rhof) += dst[i];
//            Kokkos::atomic_add(&(kfield(i, field_var::rhof)), dst[i]);
        }
    }
};
...
////        w7 = kparticles(n, particle_var::w) * q_8V;
//
//        w0 = static_cast<float>(kpart.dx(n));
//        w1 = static_cast<float>(kpart.dy(n));
//        dz = static_cast<float>(kpart.dz(n));
//        int v = kpart.i(n);
//        w7 = static_cast<float>(kpart.w(n)) * q_8V;
//
//#   define FMA( x,y,z) ((z)+(x)*(y))
//#   define FNMS(x,y,z) ((z)-(x)*(y))
//        w6=FNMS(w0,w7,w7);                    // q(1-dx)
...
//    });
    ...
    Kokkos::parallel_for("accumulate_rho_p", Kokkos::RangePolicy<Kokkos::DefaultExecutionSpace>(0, (np/8)+1), KOKKOS_LAMBDA(const int index) {
...
//      per_league++;
//    Kokkos::parallel_for("accumulate_rho_p", Kokkos::TeamPolicy<>(num_leagues, num_threads, 1), KOKKOS_LAMBDA(const KOKKOS_TEAM_POLICY_DEVICE::member_type team_member) {
//      Kokkos::parallel_for(Kokkos::TeamThreadRange(team_member, per_league), [&] (const int pindex) {
//        int n = team_member.league_rank()*per_league + pindex;
//
//        mixed_t w0, w1, w2, w3, w4, w5, w6, w7, dz;
//
//        w0 = kpart.dx(n);
...
//        int same_cell = 0;
//#ifdef __CUDA_ARCH__
//        __match_all_sync(0xffffffff, v, &same_cell);
//#endif
//        if(same_cell) {
//#ifdef __CUDA_ARCH__
//          for(int i=16; i>0; i=i/2) {
//            w0 += __shfl_down_sync(0xffffffff, w0, i);
//            w1 += __shfl_down_sync(0xffffffff, w1, i);
//            w2 += __shfl_down_sync(0xffffffff, w2, i);
//            w3 += __shfl_down_sync(0xffffffff, w3, i);
//            w4 += __shfl_down_sync(0xffffffff, w4, i);
//            w5 += __shfl_down_sync(0xffffffff, w5, i);
//            w6 += __shfl_down_sync(0xffffffff, w6, i);
//            w7 += __shfl_down_sync(0xffffffff, w7, i);
//          }
...
//    Kokkos::parallel_for("accumulate_rho_p", Kokkos::RangePolicy<Kokkos::DefaultExecutionSpace>(0, np/2), KOKKOS_LAMBDA(const int n) {
////        float w0, w1, w2, w3, w4, w5, w6, w7, dz;
//        packed_t w0, w1, w2, w3, w4, w5, w6, w7, dz;
//
////        w0 = kparticles(n, particle_var::dx);
////        w1 = kparticles(n, particle_var::dy);
////        dz = kparticles(n, particle_var::dz);
...
////        w7 = kparticles(n, particle_var::w) * q_8V;
//
//        w0 = packed_t(kpart.dx(n*2), kpart.dx(n*2+1));
//        w1 = packed_t(kpart.dy(n*2), kpart.dy(n*2+1));
//        dz = packed_t(kpart.dz(n*2), kpart.dz(n*2+1));
//        int v0 = kpart.i(n*2);
//        int v1 = kpart.i(n*2+1);
//#if defined PARTICLE_WEIGHT_FLOAT
//        w7 = packed_t(kpart.w(n*2), kpart.w(n*2+1)) * packed_t(q_8V);
//#elif defined PARTICLE_WEIGHT_SHORT
//        w7 = packed_t(kpart.w(n*2)*sp_w, kpart.w(n*2+1)*sp_w) * packed_t(q_8V);
//#elif defined PARTICLE_WEIGHT_CONSTANT
//        w7 = packed_t(sp_w) * packed_t(q_8V);
//#endif
//
//#   define FMA( x,y,z) ((z)+(x)*(y))
//#   define FNMS(x,y,z) ((z)-(x)*(y))
...
//        auto scatter_view_access = scatter_view.access();
//
//        scatter_view_access(v0,         field_var::rhof) += w0.low2float();
//        scatter_view_access(v0+1,       field_var::rhof) += w1.low2float();
//        scatter_view_access(v0+sy,      field_var::rhof) += w2.low2float();
//        scatter_view_access(v0+sy+1,    field_var::rhof) += w3.low2float();
//        scatter_view_access(v0+sz,      field_var::rhof) += w4.low2float();
//        scatter_view_access(v0+sz+1,    field_var::rhof) += w5.low2float();
//        scatter_view_access(v0+sz+sy,   field_var::rhof) += w6.low2float();
//        scatter_view_access(v0+sz+sy+1, field_var::rhof) += w7.low2float();
//
//        scatter_view_access(v1,         field_var::rhof) += w0.high2float();
//        scatter_view_access(v1+1,       field_var::rhof) += w1.high2float();
//        scatter_view_access(v1+sy,      field_var::rhof) += w2.high2float();
//        scatter_view_access(v1+sy+1,    field_var::rhof) += w3.high2float();
//        scatter_view_access(v1+sz,      field_var::rhof) += w4.high2float();
//        scatter_view_access(v1+sz+1,    field_var::rhof) += w5.high2float();
//        scatter_view_access(v1+sz+sy,   field_var::rhof) += w6.high2float();
//        scatter_view_access(v1+sz+sy+1, field_var::rhof) += w7.high2float();
//
////        Kokkos::atomic_add(&kfield(v0,         field_var::rhof), w0.low2float());
////        Kokkos::atomic_add(&kfield(v0+1,       field_var::rhof), w1.low2float());
////        Kokkos::atomic_add(&kfield(v0+sy,      field_var::rhof), w2.low2float());
////        Kokkos::atomic_add(&kfield(v0+sy+1,    field_var::rhof), w3.low2float());
////        Kokkos::atomic_add(&kfield(v0+sz,      field_var::rhof), w4.low2float());
////        Kokkos::atomic_add(&kfield(v0+sz+1,    field_var::rhof), w5.low2float());
////        Kokkos::atomic_add(&kfield(v0+sy+sz,   field_var::rhof), w6.low2float());
////        Kokkos::atomic_add(&kfield(v0+sy+sz+1, field_var::rhof), w7.low2float());
////
////        Kokkos::atomic_add(&kfield(v1,         field_var::rhof), w0.high2float());
////        Kokkos::atomic_add(&kfield(v1+1,       field_var::rhof), w1.high2float());
////        Kokkos::atomic_add(&kfield(v1+sy,      field_var::rhof), w2.high2float());
////        Kokkos::atomic_add(&kfield(v1+sy+1,    field_var::rhof), w3.high2float());
////        Kokkos::atomic_add(&kfield(v1+sz,      field_var::rhof), w4.high2float());
////        Kokkos::atomic_add(&kfield(v1+sz+1,    field_var::rhof), w5.high2float());
////        Kokkos::atomic_add(&kfield(v1+sy+sz,   field_var::rhof), w6.high2float());
////        Kokkos::atomic_add(&kfield(v1+sy+sz+1, field_var::rhof), w7.high2float());
//    });
//
    Kokkos::Experimental::contribute(kfield, scatter_view);
