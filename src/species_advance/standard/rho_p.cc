...
//    const int per_team = 1024;
//    int league_size = np/per_team;
//    if(per_team*league_size < np) {
//      league_size += 1;
//    }
//    using MemberType = Kokkos::TeamPolicy<>::member_type;
//    Kokkos::parallel_for("accumulate_rho_p", Kokkos::TeamPolicy<>(league_size, 1), 
//    KOKKOS_LAMBDA(const MemberType team_member) {
//      Kokkos::parallel_for(Kokkos::TeamThreadRange(team_member, 1),
//      [&] (const int idx) {
//        Kokkos::parallel_for(Kokkos::ThreadVectorRange(team_member, per_team), 
//        [&] (const uint64_t i) {
//          float w0, w1, w2, w3, w4, w5, w6, w7, dz;
//          const int n = team_member.team_rank()*per_team + i;
