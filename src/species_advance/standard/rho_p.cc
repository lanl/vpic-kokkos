...
//  accum_rho_p_reduce(k_field_t& k_f_, k_particles_t& k_p_, k_particles_i_t& k_p_i_, 
//                     int sy_, int sz_, float q_8V_, int np_, int nv_) : kfield(k_f_), kparticles(k_p_), kparticles_i(k_p_i_), sy(sy_), sz(sz_), q_8V(q_8V_), np(np_), value_count(nv_) {kfield = k_f_;}
//
//  KOKKOS_INLINE_FUNCTION
...
//
//  KOKKOS_INLINE_FUNCTION
//  accum_rho_p(k_field_sa_t& f_sa_, k_particles_t& p_, k_particles_i_t& p_i_, 
//              int sy_, int sz_, float q_8V_, int np_) : kfield(f_sa_), 
//                                                        kparticles(p_), 
//                                                        kparticles_i(p_i_), 
//                                                        sy(sy_), sz(sz_), 
//                                                        q_8V(q_8V_), np(np_) {}
...
//# undef FMA
//
//    auto field_access = kfield.access();
//
//    field_access(v,         field_var::rhof) += w0;
//    field_access(v+1,       field_var::rhof) += w1;
//    field_access(v+sy,      field_var::rhof) += w2;
//    field_access(v+sy+1,    field_var::rhof) += w3;
//    field_access(v+sz,      field_var::rhof) += w4;
//    field_access(v+sz+1,    field_var::rhof) += w5;
//    field_access(v+sz+sy,   field_var::rhof) += w6;
//    field_access(v+sz+sy+1, field_var::rhof) += w7;
//  }
//};
...
k_accumulate_rhob( k_field_t& kfield, 
                   ...
                   const grid_t* RESTRICT g, const float qsp, const int nm) {
  int sy = g->sy, sz = g->sz;
  float r8V = g->r8V;
  int nx = g->nx;
  int ny = g->ny;
  ...
  accum_rhob accum_rhob_kernel(kfield, kpart, kpart_i, k_part_movers_i, 
                               qsp, r8V, nx, ny, nz, sy, sz);
  Kokkos::parallel_for("accumulate_rhob", Kokkos::RangePolicy<>(0,nm), accum_rhob_kernel);
}
