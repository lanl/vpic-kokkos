...
int
hyb_move_p( particle_t       * ALIGNED(128) p0,
  ...
  float w0,wx,wy,wz,wmx,wmy,wmz,ux,uy,uz,u,absdisp,x_half,y_half,z_half,fracdt;
  const float one=1., two=2., three=3., four=4.0, half=0.5;
  ...
  for(;;) {
	...
	// Accumulate if inbnds
	a  = (float *)( a0 + p->i );              // Get accumulator
	
	// Compute coefficients
	
	w0 =  q*two*( three - x_half*x_half - y_half*y_half - z_half*z_half );
	wx =  q*( x_half + one )*( x_half + one ); 
	wy =  q*( y_half + one )*( y_half + one ); 
	wz =  q*( z_half + one )*( z_half + one ); 
	wmx =  q*( x_half - one )*( x_half - one ); 
	wmy =  q*( y_half - one )*( y_half - one ); 
	wmz =  q*( z_half - one )*( z_half - one ); 
...
#  define ACCUMULATE_J(X,offset)					\
	u=p->u##X;							\
	  a[0+offset]+= w0*u;						\
	  a[1+offset]+= wmx*u;  a[2+offset]+= wmy*u;  a[3+offset]+= wmz*u; \
	  a[4+offset]+= wx*u; a[5+offset]+= wy*u; a[6+offset]+= wz*u
...
#     undef ACCUMULATE_J
	//	 std::cout << "moved a particle  "  << std::endl  ;
