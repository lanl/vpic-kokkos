...
void
hyb_advance_p_pipeline( advance_p_pipeline_args_t * args,
                    int pipeline_rank,
                    int n_pipeline ) {
...
// ARI: accumulate J if midpoint in cell
...
#     define ACCUMULATE_J(X,offset)					               \
      a[0+offset]+= w0*p->u##X;						               \
      a[1+offset]+= wmx*p->u##X; a[2+offset]+= wmy*p->u##X; a[3+offset]+= wmz*p->u##X; \
      a[4+offset]+= wx*p->u##X;  a[5+offset]+= wy*p->u##X;  a[6+offset]+= wz*p->u##X
...
#     undef ACCUMULATE_J
    ...
    if(  dx<=one &&  dy<=one &&  dz<=one &&   // Check if inbnds
      ...
      local_pm->dispz = uz;
      local_pm->i     = p - p0;
      //      	  std::cout << "moving po  " << std::endl  ;


      if( hyb_move_p( p0, local_pm, a0, g, qsp ) ) { // Unlikely
        if( nm<max_nm ) {
	  pm[nm++] = local_pm[0];
        }
        else {
...
void
hyb_advance_p( /**/  species_t            * RESTRICT sp,
  ...
  EXEC_PIPELINES( hyb_advance_p, args, 0 );
  WAIT_PIPELINES();
