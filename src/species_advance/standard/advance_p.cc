...
#ifdef FIELD_IONIZATION
void
advance_p_kokkos_unified_ionize(
        k_particles_t& k_particles,
        k_particles_i_t& k_particles_i,
        k_particle_copy_t& k_particle_copy,
        k_particle_i_copy_t& k_particle_i_copy,
	  ...
	  short int N_ionization_levels = sp->ionization_energy.extent(0);
	   ...
	   // initialize variables for the while loop
	    ...
	    K                    = floor(epsilon_au/omega_au)+1; // number of photons required for multiphoton ionization          
	      ...
	      float Gamma_MPI;
              if (K < 15){  // If K! is too large then the multiphoton ionisation rate is zero
                ...
                Gamma_MPI = sigma_K_au * pow(flux, K) *Gamma_conversion; // SI units                                                                                                                                               
              } else {
                Gamma_MPI = 0; // SI units                                                                                                                                                                                         
              }
        ...
        const int nz)
{
  ...
  Kokkos::parallel_for("advance_p", range_policy, KOKKOS_LAMBDA (size_t p_index) {
    ...
    int   ii   = pii;
#ifdef FIELD_IONIZATION    
    const float qdt_2mc = p_q * dt_2mc;
#endif    
...
//           accumulator_array_t * RESTRICT aa,
  ...
  float cdt_dz   = sp->g->cvac*sp->g->dt*sp->g->rdz;
    ...
    #ifdef FIELD_IONIZATION
       #define ADVANCE_P_IONIZE advance_p_kokkos_gpu_ionize
    #endif
    ...
    #ifdef FIELD_IONIZATION
       #define ADVANCE_P_IONIZE advance_p_kokkos_unified_ionize
    #endif
