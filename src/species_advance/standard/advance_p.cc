...
//  #define p_dx    k_particles.dx(p_index)
//  #define p_dy    k_particles.dy(p_index)
//  #define p_dz    k_particles.dz(p_index)
//  #define p_ux    k_particles.ux(p_index)
//  #define p_uy    k_particles.uy(p_index)
//  #define p_uz    k_particles.uz(p_index)
//#if defined PARTICLE_WEIGHT_FLOAT
//  #define p_w     k_particles.w(p_index)
//#elif defined PARTICLE_WEIGHT_SHORT
//  #define p_w     k_particles.w(p_index)*sp_w
//#elif defined PARTICLE_WEIGHT_CONSTANT
//  #define p_w     sp_w
//#endif
//  #define pii     k_particles.i(p_index)
//
...
//
////  Kokkos::parallel_for("advance_p", Kokkos::RangePolicy<>(0, num_chunks),
////  KOKKOS_LAMBDA (size_t chunk)
////  {
...
////      p_uy = uy[lane];
////      p_uz = uz[lane];
//      k_particles.set_ux(ux[lane], p_index);
//      k_particles.set_uy(uy[lane], p_index);
//      k_particles.set_uz(uz[lane], p_index);
//    }
//
//#pragma omp simd
//    for(int lane=0; lane<num_lanes; lane++) {
...
////      p_dy = v4[lane];
////      p_dz = v5[lane];
//      k_particles.set_dx(v3[lane], p_index);
//      k_particles.set_dy(v4[lane], p_index);
//      k_particles.set_dz(v5[lane], p_index);
//      dx[lane] = v0[lane];
//      dy[lane] = v1[lane];
//      dz[lane] = v2[lane];
...
//      	                   k_accumulators_sa, g, k_neighbors, rangel, rangeh, qsp, sp_w ) ) { // Unlikely
//          if( k_nm(0)<max_nm ) {
...
//          	k_particle_copy.dx(nm) = k_particles.dx(p_index);
//          	k_particle_copy.dy(nm) = k_particles.dy(p_index);
//          	k_particle_copy.dz(nm) = k_particles.dz(p_index);
//          	k_particle_copy.ux(nm) = k_particles.ux(p_index);
//          	k_particle_copy.uy(nm) = k_particles.uy(p_index);
//          	k_particle_copy.uz(nm) = k_particles.uz(p_index);
//          	k_particle_copy.i(nm) = k_particles.i(p_index);
//#if !defined PARTICLE_WEIGHT_CONSTANT
//          	k_particle_copy.w(nm) = k_particles.w(p_index);
//#endif
//          }
//        }
//      }
...
////      p_uy = uy;
////      p_uz = uz;
//      k_particles.set_ux(ux, p_index);
//      k_particles.set_uy(uy, p_index);
//      k_particles.set_uz(uz, p_index);
//
...
////        p_dy = v4;
////        p_dz = v5;
//        k_particles.set_dx(v3, p_index);
//        k_particles.set_dy(v4, p_index);
//        k_particles.set_dz(v5, p_index);
...
>>>>>>> 28db488 (Code changes for running on Power9 and Cuda)
//// Half 2
//void
//advance_p_kokkos(
...
//#elif defined PARTICLE_WEIGHT_SHORT
//  #define p_w     k_part.w(p_index)*sp_w
//#elif defined PARTICLE_WEIGHT_CONSTANT
//  #define p_w     sp_w
//#endif
...
////    KOKKOS_LAMBDA (size_t p_index)
//  Kokkos::parallel_for("advance_p", Kokkos::RangePolicy<>(0, (np/8)+1),
//    KOKKOS_LAMBDA (size_t index)
//    {
//for(int idx = 0; idx < 8; idx++) {
//size_t p_index = index*8 + idx;
//if(p_index < np) {
//
//    auto  k_accumulators_scatter_access = k_accumulators_sa.access();
//
...
////    mixed_t dy = p_dy;
////    mixed_t dz = p_dz;
//    mixed_t dx = k_part.get_dx(p_index); // Load position
...
////      p_dy = v4;
////      p_dz = v5;
//      k_part.set_dx(v3, p_index);                             // Store new position
//      k_part.set_dy(v4, p_index);
//      k_part.set_dz(v5, p_index);
//      dx = v0;                                // Streak midpoint
//      dy = v1;
//      dz = v2;
//      v5 = q*ux*uy*uz*one_third;              // Compute correction
...
//          k_particle_copy.set_dx(k_part.get_dx(p_index), nm);
//          k_particle_copy.set_dy(k_part.get_dy(p_index), nm);
//          k_particle_copy.set_dz(k_part.get_dz(p_index), nm);
...
//    mixed_t dz = p_dz;
    ...
    mixed_t uz   = p_uz;
    mixed_t q    = p_w;
//    mixed_t q    = k_part.w(p_index)*sp_w;
...
//    p_uz = uz;
    ...
    bool inbnds = (v3 <= 1.0);
    inbnds = inbnds && (v4<=1.0);
    inbnds = inbnds && (v5<=1.0);
    inbnds = inbnds && (-v3<=1.0);
    inbnds = inbnds && (-v4<=1.0);
    inbnds = inbnds && (-v5<=1.0);
    if( inbnds ) {
//    if(  v3<=mixed_t(one) &&  v4<=mixed_t(one) &&  v5<=mixed_t(one) &&   // Check if inbnds
...
////                             k_accumulators_sa, g, k_neighbors, rangel, rangeh, qsp ) ) { // Unlikely
//          if( move_p_kokkos(k_part, local_pm,
//                             k_accumulator, g, k_neighbors, rangel, rangeh, qsp, sp_w ) ) { // Unlikely
//            if( k_nm(0)<max_nm ) {
//              const unsigned int nm = Kokkos::atomic_fetch_add( &k_nm(0), 1 );
//              if (nm >= max_nm) Kokkos::abort("overran max_nm");
//    
