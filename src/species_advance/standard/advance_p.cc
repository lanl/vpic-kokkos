...
//  if(np%num_lanes > 0)
//    num_chunks++;
  Kokkos::TeamPolicy<> policy = Kokkos::TeamPolicy<>(num_leagues, Kokkos::AUTO(), num_lanes).set_scratch_size(1, Kokkos::PerThread(21*num_lanes*4));
//  Kokkos::TeamPolicy<> policy = Kokkos::TeamPolicy<>(num_leagues, Kokkos::AUTO(), num_lanes);
  ...
  KOKKOS_LAMBDA(const KOKKOS_TEAM_POLICY_DEVICE::member_type team_member) {
    simd_float v0(team_member.team_scratch(1));
    simd_float v1(team_member.team_scratch(1));
    simd_float v2(team_member.team_scratch(1));
    simd_float v3(team_member.team_scratch(1));
    simd_float v4(team_member.team_scratch(1));
    simd_float v5(team_member.team_scratch(1));
    simd_float dx(team_member.team_scratch(1));
    simd_float dy(team_member.team_scratch(1));
    simd_float dz(team_member.team_scratch(1));
    simd_float ux(team_member.team_scratch(1));
    simd_float uy(team_member.team_scratch(1));
    simd_float uz(team_member.team_scratch(1));
    simd_float hax(team_member.team_scratch(1));
    simd_float hay(team_member.team_scratch(1));
    simd_float haz(team_member.team_scratch(1));
    simd_float cbx(team_member.team_scratch(1));
    simd_float cby(team_member.team_scratch(1));
    simd_float cbz(team_member.team_scratch(1));
    simd_int   ii(team_member.team_scratch(1));
    simd_float q(team_member.team_scratch(1));
    simd_bool  inbnds(team_member.team_scratch(1));
...
void
advance_p_kokkos_devel_serial_chunk(
        k_particles_t& k_particles,
        k_particles_i_t& k_particles_i,
        k_particle_copy_t& k_particle_copy,
        k_particle_i_copy_t& k_particle_i_copy,
...
//      v5[lane] = ( v5[lane] & inbnds[lane] ) | ( dz[lane] & ~inbnds[lane] );
      ...
      q[lane] = inbnds[lane] ? q[lane]*qsp : 0.0;
//      q[lane] = q[lane]*qsp*static_cast<float>(inbnds[lane]);
//      q[lane] *= qsp;
      p_dx = v3[lane];
      p_dy = v4[lane];
      p_dz = v5[lane];
