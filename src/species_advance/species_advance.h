...
void
hyb_center_p( /**/  species_t            * RESTRICT sp,
          const interpolator_array_t * RESTRICT ia );
...
//ARI: hyb_uncenter does double duty--will also accumulate initial moments
void
...
double
hyb_energy_p( const species_t            * RESTRICT sp,
          const interpolator_array_t * RESTRICT ia );
...
int
hyb_move_p( particle_t       * ALIGNED(128) p0,    // Particle array
        particle_mover_t * ALIGNED(16)  m,     // Particle mover to apply
        accumulator_t    * ALIGNED(128) a0,    // Accumulator to use
        const grid_t     *              g,     // Grid parameters
        const float                     qsp ); // Species particle charge
