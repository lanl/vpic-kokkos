...
class species_t {
    public:
        ...
        float w = 0.0;                      // Species unit weight
...
//    s_midy = p_dy;
//    s_midz = p_dz;
    s_midx = k_part.get_dx(pi);
    s_midy = k_part.get_dy(pi);
    s_midz = k_part.get_dz(pi);
...
//    s_midy = p_dy;
//    s_midz = p_dz;
    s_midx = k_particles.get_dx(pi);
    s_midy = k_particles.get_dy(pi);
    s_midz = k_particles.get_dz(pi);
...
//    p_dy += s_dispy+s_dispy;
//    p_dz += s_dispz+s_dispz;
    k_particles.set_dx(static_cast<float>(k_particles.get_dx(pi))+s_dispx+s_dispx, pi);
    k_particles.set_dy(static_cast<float>(k_particles.get_dy(pi))+s_dispy+s_dispy, pi);
    k_particles.set_dz(static_cast<float>(k_particles.get_dz(pi))+s_dispz+s_dispz, pi);
