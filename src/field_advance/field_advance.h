...
typedef struct field {
  float ex,   ey,   ez,   pexx;        // Electric field and div E error
  float cbx,  cby,  cbz,  div_b_err;        // Magnetic field and div B error
  float cbx0, cby0, cbz0, tmpsm; 
  float jfx,  jfy,  jfz,  rhof;             // Ion current and charge density
  float jfxold,  jfyold,  jfzold,  rhofold; //ARI: old ion current and charge density
  float tcax, tcay, tcaz, rhob;             // TCA fields and bound charge density
  material_id ematx, ematy, ematz, nmat;    // Material at edge centers and nodes
  material_id fmatx, fmaty, fmatz, cmat;    // Material at face and cell centers
  float cbxold, cbyold, cbzold, div_e_err;       //for RK4 B solver, pexx for scalar electron pressure
  float tempx, tempy, tempz;                //""
  float pexy, pexz, peyy, peyz, pezz;       //electron pressure tensor.
  float curlbx,curlby,curlbz;               //for hyper-resistivity
  float smex, smey, smez, smcbx,smcby,smcbz;//smoothed fields for scatter.
} field_t;
...
// Smoothing interface
  ...
  void   (*smooth_eb            )( /**/  struct field_array * RESTRICT fa , const int smoothed);
  void   (*smooth_moments       )( /**/  struct field_array * RESTRICT fa );
  void   (*smooth_b             )( /**/  struct field_array * RESTRICT fa );
