...
//ARI: fix ghost E field
void
hyb_local_ghost_e( field_t      * ALIGNED(128) f,
                     const grid_t *              g ) {
  const int nx = g->nx, ny = g->ny, nz = g->nz;
  int bc, face, x, y, z;

# define HYB_GHOST_E(i,j,k,X,Y,Z)					\
  do {                                                                  \
    bc = g->bc[BOUNDARY(i,j,k)];                                        \
    if( bc<0 || bc>=world_size ) {                                      \
      face = (i+j+k)<0 ? 0 : n##X+1;                                    \
      switch(bc) {                                                      \
      case anti_symmetric_fields:					\
	X##_FACE_LOOP(face) f(x,y,z).e##X = f(x-i,y-j,z-k).e##X;	\
	X##_FACE_LOOP(face) f(x,y,z).e##Y = f(x-i,y-j,z-k).e##Y;	\
	X##_FACE_LOOP(face) f(x,y,z).e##Z = f(x-i,y-j,z-k).e##Z;	\
	X##_FACE_LOOP(face) f(x,y,z).sme##X = f(x-i,y-j,z-k).sme##X;	\
	X##_FACE_LOOP(face) f(x,y,z).sme##Y = f(x-i,y-j,z-k).sme##Y;    \
	X##_FACE_LOOP(face) f(x,y,z).sme##Z = f(x-i,y-j,z-k).sme##Z;    \
	break;    							\
      case symmetric_fields: case pmc_fields: case absorb_fields:       \
	X##_FACE_LOOP(face) f(x,y,z).e##X = 0*f(x-i,y-j,z-k).e##X;	\
	X##_FACE_LOOP(face) f(x,y,z).e##Y = 0*f(x-i,y-j,z-k).e##Y;	\
	X##_FACE_LOOP(face) f(x,y,z).e##Z = 0*f(x-i,y-j,z-k).e##Z;	\
	X##_FACE_LOOP(face) f(x,y,z).sme##X = f(x-i,y-j,z-k).sme##X;	\
	X##_FACE_LOOP(face) f(x,y,z).sme##Y = f(x-i,y-j,z-k).sme##Y;    \
	X##_FACE_LOOP(face) f(x,y,z).sme##Z = f(x-i,y-j,z-k).sme##Z;    \
	  break;							\
  ...
  HYB_GHOST_E(-1, 0, 0,x,y,z);
  HYB_GHOST_E( 0,-1, 0,y,z,x);
  HYB_GHOST_E( 0, 0,-1,z,x,y);
  HYB_GHOST_E( 1, 0, 0,x,y,z);
  HYB_GHOST_E( 0, 1, 0,y,z,x);
  HYB_GHOST_E( 0, 0, 1,z,x,y);
...
//ARI: fix ghost B field
void
hyb_local_init_ghost_b( field_t      * ALIGNED(128) f,
                     const grid_t *              g ) {
  const int nx = g->nx, ny = g->ny, nz = g->nz;
  int bc, face, x, y, z;

# define HYB_INIT_GHOST_B(i,j,k,X,Y,Z)					\
  do {                                                                  \
    bc = g->bc[BOUNDARY(i,j,k)];                                        \
    if( bc<0 || bc>=world_size ) {                                      \
      face = (i+j+k)<0 ? 0 : n##X+1;                                    \
      switch(bc) {                                                      \
      case anti_symmetric_fields:                                       \
	X##_FACE_LOOP(face) f(x,y,z).cb##X##old = f(x-i,y-j,z-k).cb##X;	\
	X##_FACE_LOOP(face) f(x,y,z).cb##Y##old = f(x-i,y-j,z-k).cb##Y;	\
	X##_FACE_LOOP(face) f(x,y,z).cb##Z##old = f(x-i,y-j,z-k).cb##Z;	\
	break;    							\
      case symmetric_fields: case pmc_fields: case absorb_fields:       \
	X##_FACE_LOOP(face) f(x,y,z).cb##X##old = f(x-i,y-j,z-k).cb##X;	\
	X##_FACE_LOOP(face) f(x,y,z).cb##Y##old = f(x-i,y-j,z-k).cb##Y;	\
  ...
  HYB_INIT_GHOST_B(-1, 0, 0,x,y,z);
  HYB_INIT_GHOST_B( 0,-1, 0,y,z,x);
  HYB_INIT_GHOST_B( 0, 0,-1,z,x,y);
  HYB_INIT_GHOST_B( 1, 0, 0,x,y,z);
  HYB_INIT_GHOST_B( 0, 1, 0,y,z,x);
  HYB_INIT_GHOST_B( 0, 0, 1,z,x,y);
...
//ARI: fix ghost B field
void
hyb_local_ghost_b( field_t      * ALIGNED(128) f,
                     const grid_t *              g ) {
  const int nx = g->nx, ny = g->ny, nz = g->nz;
  int bc, face, x, y, z;

# define HYB_GHOST_B(i,j,k,X,Y,Z)					\
  do {                                                                  \
    bc = g->bc[BOUNDARY(i,j,k)];                                        \
    if( bc<0 || bc>=world_size ) {                                      \
      face = (i+j+k)<0 ? 0 : n##X+1;                                    \
      switch(bc) {                                                      \
      case anti_symmetric_fields:                                       \
	X##_FACE_LOOP(face) f(x,y,z).cb##X =  f(x-i,y-j,z-k).cb##X;	\
	X##_FACE_LOOP(face) f(x,y,z).cb##Y =  f(x-i,y-j,z-k).cb##Y;	\
	X##_FACE_LOOP(face) f(x,y,z).cb##Z =  f(x-i,y-j,z-k).cb##Z;	\
	X##_FACE_LOOP(face) f(x,y,z).smcb##X =  f(x-i,y-j,z-k).smcb##X;	\
	X##_FACE_LOOP(face) f(x,y,z).smcb##Y =  f(x-i,y-j,z-k).smcb##Y; \
	X##_FACE_LOOP(face) f(x,y,z).smcb##Z =  f(x-i,y-j,z-k).smcb##Z; \
	    break;    							\
      case symmetric_fields: case pmc_fields: case absorb_fields:       \
	/*X##_FACE_LOOP(face) f(x,y,z).cb##X = 2.0*f(x,y,z).cb##X##old - f(x-i,y-j,z-k).cb##X;*/ \
	X##_FACE_LOOP(face) f(x,y,z).cb##X =  f(x-i,y-j,z-k).cb##X;	\
	X##_FACE_LOOP(face) f(x,y,z).cb##Y =  f(x-i,y-j,z-k).cb##Y;	\
	X##_FACE_LOOP(face) f(x,y,z).cb##Z =  f(x-i,y-j,z-k).cb##Z;	\
	X##_FACE_LOOP(face) f(x,y,z).smcb##X =  f(x-i,y-j,z-k).smcb##X; \
  ...
  HYB_GHOST_B(-1, 0, 0,x,y,z);
  HYB_GHOST_B( 0,-1, 0,y,z,x);
  HYB_GHOST_B( 0, 0,-1,z,x,y);
  HYB_GHOST_B( 1, 0, 0,x,y,z);
  HYB_GHOST_B( 0, 1, 0,y,z,x);
  HYB_GHOST_B( 0, 0, 1,z,x,y);
...
//ARI: add ghost cell to adjacent local BC cell, then set ghost to 0
void
hyb_local_adjust_rhof( field_t      * ALIGNED(128) f,
                   const grid_t *              g ) {
  const int nx = g->nx, ny = g->ny, nz = g->nz;
  int bc, face, x, y, z;

# define HYB_ADJUST_RHOF(i,j,k,X,Y,Z)					\
  do {                                                                  \
    bc = g->bc[BOUNDARY(i,j,k)];                                        \
    if( bc<0 || bc>=world_size ) {                                      \
      face  = (i+j+k)<0 ? 1 : n##X;					\
      switch(bc) {							\
      case anti_symmetric_fields:					\
	X##_FACE_LOOP(face) f(x,y,z).jfx  += f(x+i,y+j,z+k).jfx;	\
	X##_FACE_LOOP(face) f(x,y,z).jfy  += f(x+i,y+j,z+k).jfy;        \
	X##_FACE_LOOP(face) f(x,y,z).jfz  += f(x+i,y+j,z+k).jfz;        \
	X##_FACE_LOOP(face) f(x,y,z).rhof += f(x+i,y+j,z+k).rhof;	\
	X##_FACE_LOOP(face) f(x+i,y+j,z+k).jfx  = 0;			\
	X##_FACE_LOOP(face) f(x+i,y+j,z+k).jfy  = 0;			\
	X##_FACE_LOOP(face) f(x+i,y+j,z+k).jfz  = 0;			\
	X##_FACE_LOOP(face) f(x+i,y+j,z+k).rhof = 0;			\
       case symmetric_fields: case pmc_fields: case absorb_fields:	\
	X##_FACE_LOOP(face) f(x,y,z).jfx  += f(x+i,y+j,z+k).jfx;	\
	X##_FACE_LOOP(face) f(x,y,z).jfy  += f(x+i,y+j,z+k).jfy;	\
	X##_FACE_LOOP(face) f(x,y,z).jfz  += f(x+i,y+j,z+k).jfz;	\
  ...
  HYB_ADJUST_RHOF(-1, 0, 0,x,y,z);
  HYB_ADJUST_RHOF( 0,-1, 0,y,z,x);
  HYB_ADJUST_RHOF( 0, 0,-1,z,x,y);
  HYB_ADJUST_RHOF( 1, 0, 0,x,y,z);
  HYB_ADJUST_RHOF( 0, 1, 0,y,z,x);
  HYB_ADJUST_RHOF( 0, 0, 1,z,x,y);
}
...
//ARI:  ghost cell 
void
hyb_local_ghost_rhof( field_t      * ALIGNED(128) f,
                   const grid_t *              g ) {
  const int nx = g->nx, ny = g->ny, nz = g->nz;
  int bc, face, x, y, z;

# define HYB_GHOST_RHOF(i,j,k,X,Y,Z)					\
  do {                                                                  \
    bc = g->bc[BOUNDARY(i,j,k)];                                        \
    if( bc<0 || bc>=world_size ) {                                      \
      face  = (i+j+k)<0 ? 0 : n##X+1;					\
      switch(bc) {							\
      case anti_symmetric_fields:					\
	X##_FACE_LOOP(face) f(x,y,z).jf##X  = f(x-i,y-j,z-k).jf##X;	\
	X##_FACE_LOOP(face) f(x,y,z).jf##Y  = f(x-i,y-j,z-k).jf##Y;	\
	X##_FACE_LOOP(face) f(x,y,z).jf##Z  = f(x-i,y-j,z-k).jf##Z;	\
	X##_FACE_LOOP(face) f(x,y,z).rhof =  f(x-i,y-j,z-k).rhof;	\
       case symmetric_fields: case pmc_fields: case absorb_fields:	\
	X##_FACE_LOOP(face) f(x,y,z).jfx  = f(x-i,y-j,z-k).jfx;	        \
	X##_FACE_LOOP(face) f(x,y,z).jfy  = f(x-i,y-j,z-k).jfy;	        \
  ...
  HYB_GHOST_RHOF(-1, 0, 0,x,y,z);
  HYB_GHOST_RHOF( 0,-1, 0,y,z,x);
  HYB_GHOST_RHOF( 0, 0,-1,z,x,y);
  HYB_GHOST_RHOF( 1, 0, 0,x,y,z);
  HYB_GHOST_RHOF( 0, 1, 0,y,z,x);
  HYB_GHOST_RHOF( 0, 0, 1,z,x,y);
}
