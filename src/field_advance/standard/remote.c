...
//ARI: for  cell-centered hybrid solver
//add ghost cell contents to edge cells
void
begin_remote_edge_rho( field_t      * ALIGNED(128) field,
                           const grid_t *              g ) {
...
# define BEGIN_RECV(i,j,k,X,Y,Z) \
  begin_recv_port(i,j,k,( 1 + 4*(n##Y)*(n##Z) )*sizeof(float),g)
...
# define BEGIN_SEND(i,j,k,X,Y,Z) BEGIN_PRIMITIVE {			\
    size = (1 + 4*(n##Y)*(n##Z) )*sizeof(float);			\
    p = (float *)size_send_port( i, j, k, size, g );			\
    if( p ) {								\
      (*(p++)) = g->d##X;						\
	face = (i+j+k)<0 ? 0 : n##X+1;					\
	X##_FACE_LOOP(face) (*(p++)) = field(x,y,z).jfx;		\
	X##_FACE_LOOP(face) (*(p++)) = field(x,y,z).jfy;		\
	X##_FACE_LOOP(face) (*(p++)) = field(x,y,z).jfz;		\
	X##_FACE_LOOP(face) (*(p++)) = field(x,y,z).rhof;	        \
...
//ARI: for cell-centered hybrid
void
end_remote_edge_rho( field_t      * ALIGNED(128) field,
                         const grid_t *              g ) {
  const int nx = g->nx, ny = g->ny, nz = g->nz;
  int face, x, y, z;
  float *p, lw;
...
# define END_RECV(i,j,k,X,Y,Z) BEGIN_PRIMITIVE {                        \
    ...
    if( p ) {                                                           \
      lw = (*(p++));                 /* Remote g->d##X */               \
      face = (i+j+k)<0 ? n##X : 1;  /*add ghosts to edge cells*/	\
      X##_FACE_LOOP(face)  field(x,y,z).jfx += (*(p++)); 		\
      X##_FACE_LOOP(face)  field(x,y,z).jfy += (*(p++)); 		\
      X##_FACE_LOOP(face)  field(x,y,z).jfz += (*(p++)); 		\
      X##_FACE_LOOP(face)  field(x,y,z).rhof += (*(p++)); 		\
    }                                                                   \
  } END_PRIMITIVE
...
//ARI: for  cell-centered hybrid solver
void
begin_remote_ghost_rho( field_t      * ALIGNED(128) field,
                           const grid_t *              g ) {
  const int nx = g->nx, ny = g->ny, nz = g->nz;
  int size, face, x, y, z;
  float *p;
...
//ARI: for cell-centered hybrid
void
end_remote_ghost_rho( field_t      * ALIGNED(128) field,
                         const grid_t *              g ) {
  const int nx = g->nx, ny = g->ny, nz = g->nz;
  int face, x, y, z;
  float *p, lw;
...
//ARI for  cell-centered hybrid solver
void
begin_remote_smooth_rho( field_t      * ALIGNED(128) field,
                           const grid_t *              g ) {
  const int nx = g->nx, ny = g->ny, nz = g->nz;
  int size, face, x, y, z;
  float *p;
...
//ARI for cell-centered hybrid
void
end_remote_smooth_rho( field_t      * ALIGNED(128) field,
                         const grid_t *              g ) {
  const int nx = g->nx, ny = g->ny, nz = g->nz;
  int face, x, y, z;
  float *p, lw;
...
//ARI for  cell-centered hybrid solver
void
begin_remote_smooth_b( field_t      * ALIGNED(128) field,
                           const grid_t *              g ) {
  const int nx = g->nx, ny = g->ny, nz = g->nz;
  int size, face, x, y, z;
  float *p;
...
# define BEGIN_RECV(i,j,k,X,Y,Z) \
  begin_recv_port(i,j,k,( 1 + 3*(n##Y)*(n##Z) )*sizeof(float),g)
  BEGIN_RECV(-1, 0, 0,x,y,z);
  BEGIN_RECV( 0,-1, 0,y,z,x);
  BEGIN_RECV( 0, 0,-1,z,x,y);
  BEGIN_RECV( 1, 0, 0,x,y,z);
...
# define BEGIN_SEND(i,j,k,X,Y,Z) BEGIN_PRIMITIVE {          \
    size = (1 + 3*(n##Y)*(n##Z) )*sizeof(float);	    \
    p = (float *)size_send_port( i, j, k, size, g );        \
    if( p ) {                                               \
      (*(p++)) = g->d##X;				    \
      face = (i+j+k)<0 ? 1 : n##X;			    \
      X##_FACE_LOOP(face) (*(p++)) = field(x,y,z).smcbx;      \
      X##_FACE_LOOP(face) (*(p++)) = field(x,y,z).smcby;      \
      X##_FACE_LOOP(face) (*(p++)) = field(x,y,z).smcbz;      \
      begin_send_port( i, j, k, size, g );                  \
    }                                                       \
  } END_PRIMITIVE
  BEGIN_SEND(-1, 0, 0,x,y,z);
...
//ARI for cell-centered hybrid
void
end_remote_smooth_b( field_t      * ALIGNED(128) field,
                         const grid_t *              g ) {
  const int nx = g->nx, ny = g->ny, nz = g->nz;
  int face, x, y, z;
  float *p, lw;
...
# define END_RECV(i,j,k,X,Y,Z) BEGIN_PRIMITIVE {                        \
    ...
    if( p ) {                                                           \
      lw = (*(p++));                 /* Remote g->d##X */               \
      face = (i+j+k)<0 ? n##X+1 : 0;                                    \
      X##_FACE_LOOP(face)  field(x,y,z).smcbx = (*(p++)); 		\
      X##_FACE_LOOP(face)  field(x,y,z).smcby = (*(p++)); 	  	\
      X##_FACE_LOOP(face)  field(x,y,z).smcbz = (*(p++));  		\
    }                                                                   \
  } END_PRIMITIVE
  END_RECV(-1, 0, 0,x,y,z);
  END_RECV( 0,-1, 0,y,z,x);
...
//ARI for  cell-centered hybrid solver
void
begin_remote_smooth_e( field_t      * ALIGNED(128) field,
                           const grid_t *              g ) {
  const int nx = g->nx, ny = g->ny, nz = g->nz;
  int size, face, x, y, z;
  float *p;
...
# define BEGIN_SEND(i,j,k,X,Y,Z) BEGIN_PRIMITIVE {          \
    ...
    if( p ) {                                               \
      (*(p++)) = g->d##X;				    \
      face = (i+j+k)<0 ? 1 : n##X;			    \
      X##_FACE_LOOP(face) (*(p++)) = field(x,y,z).smex;      \
      X##_FACE_LOOP(face) (*(p++)) = field(x,y,z).smey;      \
      X##_FACE_LOOP(face) (*(p++)) = field(x,y,z).smez;      \
      begin_send_port( i, j, k, size, g );                  \
    }                                                       \
  } END_PRIMITIVE
  BEGIN_SEND(-1, 0, 0,x,y,z);
...
//ARI for cell-centered hybrid
void
end_remote_smooth_e( field_t      * ALIGNED(128) field,
                         const grid_t *              g ) {
  const int nx = g->nx, ny = g->ny, nz = g->nz;
  int face, x, y, z;
  float *p, lw;
...
# define END_RECV(i,j,k,X,Y,Z) BEGIN_PRIMITIVE {                        \
    ...
    if( p ) {                                                           \
      lw = (*(p++));                 /* Remote g->d##X */               \
      face = (i+j+k)<0 ? n##X+1 : 0;                                    \
      X##_FACE_LOOP(face)  field(x,y,z).smex = (*(p++)); 		\
      X##_FACE_LOOP(face)  field(x,y,z).smey = (*(p++)); 	  	\
      X##_FACE_LOOP(face)  field(x,y,z).smez = (*(p++));  		\
    }                                                                   \
  } END_PRIMITIVE
  END_RECV(-1, 0, 0,x,y,z);
  END_RECV( 0,-1, 0,y,z,x);
...
//ARI for  cell-centered hybrid solver
void
begin_remote_ghost_b( field_t      * ALIGNED(128) field,
                           const grid_t *              g ) {
  const int nx = g->nx, ny = g->ny, nz = g->nz;
  int size, face, x, y, z;
  float *p;
...
# define BEGIN_SEND(i,j,k,X,Y,Z) BEGIN_PRIMITIVE {          \
    ...
    if( p ) {                                               \
      (*(p++)) = g->d##X;				    \
      face = (i+j+k)<0 ? 1 : n##X;			    \
      X##_FACE_LOOP(face) (*(p++)) = field(x,y,z).cbx;      \
      X##_FACE_LOOP(face) (*(p++)) = field(x,y,z).cby;      \
      X##_FACE_LOOP(face) (*(p++)) = field(x,y,z).cbz;      \
      begin_send_port( i, j, k, size, g );                  \
    }                                                       \
  } END_PRIMITIVE
  BEGIN_SEND(-1, 0, 0,x,y,z);
...
//ARI for cell-centered hybrid
void
end_remote_ghost_b( field_t      * ALIGNED(128) field,
                         const grid_t *              g ) {
  const int nx = g->nx, ny = g->ny, nz = g->nz;
  int face, x, y, z;
  float *p, lw;
...
# define END_RECV(i,j,k,X,Y,Z) BEGIN_PRIMITIVE {                        \
    ...
    if( p ) {                                                           \
      lw = (*(p++));                 /* Remote g->d##X */               \
      face = (i+j+k)<0 ? n##X+1 : 0;                                    \
      X##_FACE_LOOP(face)  field(x,y,z).cbx = (*(p++)); 		\
      X##_FACE_LOOP(face)  field(x,y,z).cby = (*(p++)); 	  	\
      X##_FACE_LOOP(face)  field(x,y,z).cbz = (*(p++));  		\
    }                                                                   \
  } END_PRIMITIVE
  END_RECV(-1, 0, 0,x,y,z);
  END_RECV( 0,-1, 0,y,z,x);
...
//ARI for  cell-centered hybrid solver
void
begin_remote_curlb( field_t      * ALIGNED(128) field,
                           const grid_t *              g ) {
  const int nx = g->nx, ny = g->ny, nz = g->nz;
  int size, face, x, y, z;
  float *p;
...
# define BEGIN_SEND(i,j,k,X,Y,Z) BEGIN_PRIMITIVE {          \
    ...
    if( p ) {                                               \
      (*(p++)) = g->d##X;				    \
      face = (i+j+k)<0 ? 1 : n##X;			    \
      X##_FACE_LOOP(face) (*(p++)) = field(x,y,z).curlbx;      \
      X##_FACE_LOOP(face) (*(p++)) = field(x,y,z).curlby;      \
      X##_FACE_LOOP(face) (*(p++)) = field(x,y,z).curlbz;      \
      begin_send_port( i, j, k, size, g );                  \
    }                                                       \
  } END_PRIMITIVE
  BEGIN_SEND(-1, 0, 0,x,y,z);
...
//ARI for cell-centered hybrid
void
end_remote_curlb( field_t      * ALIGNED(128) field,
                         const grid_t *              g ) {
  const int nx = g->nx, ny = g->ny, nz = g->nz;
  int face, x, y, z;
  float *p, lw;
...
# define END_RECV(i,j,k,X,Y,Z) BEGIN_PRIMITIVE {                        \
    ...
    if( p ) {                                                           \
      lw = (*(p++));                 /* Remote g->d##X */               \
      face = (i+j+k)<0 ? n##X+1 : 0;                                    \
      X##_FACE_LOOP(face)  field(x,y,z).curlbx = (*(p++)); 		\
      X##_FACE_LOOP(face)  field(x,y,z).curlby = (*(p++)); 	  	\
      X##_FACE_LOOP(face)  field(x,y,z).curlbz = (*(p++));  		\
    }                                                                   \
  } END_PRIMITIVE
  END_RECV(-1, 0, 0,x,y,z);
  END_RECV( 0,-1, 0,y,z,x);
...
//ARI for  cell-centered hybrid solver
void
begin_remote_ghost_e( field_t      * ALIGNED(128) field,
                           const grid_t *              g ) {
  const int nx = g->nx, ny = g->ny, nz = g->nz;
  int size, face, x, y, z;
  float *p;
...
# define BEGIN_SEND(i,j,k,X,Y,Z) BEGIN_PRIMITIVE {          \
    ...
    if( p ) {                                               \
      (*(p++)) = g->d##X;				    \
      face = (i+j+k)<0 ? 1 : n##X;			    \
      X##_FACE_LOOP(face) (*(p++)) = field(x,y,z).ex;       \
      X##_FACE_LOOP(face) (*(p++)) = field(x,y,z).ey;       \
      X##_FACE_LOOP(face) (*(p++)) = field(x,y,z).ez;       \
      begin_send_port( i, j, k, size, g );                  \
    }                                                       \
  } END_PRIMITIVE
  BEGIN_SEND(-1, 0, 0,x,y,z);
...
//ARI for cell-centered hybrid
void
end_remote_ghost_e( field_t      * ALIGNED(128) field,
                         const grid_t *              g ) {
  const int nx = g->nx, ny = g->ny, nz = g->nz;
  int face, x, y, z;
  float *p, lw;
...
# define END_RECV(i,j,k,X,Y,Z) BEGIN_PRIMITIVE {                        \
    ...
    if( p ) {                                                           \
      lw = (*(p++));                 /* Remote g->d##X */               \
      face = (i+j+k)<0 ? n##X+1 : 0;					\
      X##_FACE_LOOP(face)  field(x,y,z).ex = (*(p++));	        	\
      X##_FACE_LOOP(face)  field(x,y,z).ey = (*(p++));    		\
      X##_FACE_LOOP(face)  field(x,y,z).ez = (*(p++));		        \
    }                                                                   \
  } END_PRIMITIVE
  END_RECV(-1, 0, 0,x,y,z);
  END_RECV( 0,-1, 0,y,z,x);
