...
//          // for efficiency
//          //accumulate_rhob( f, p0+i, g, sp_q );
//          k_accumulate_rhob_single_cpu( fa->k_f_h, sp->k_p_h, sp->k_p_i_h, i, g, sp_q);
//          goto backfill;
//        }
//
//        // Send to a neighboring node
...
////#         else
//          pi->dx=p0(i, particle_var::dx); 
//          pi->dy=p0(i, particle_var::dy); 
//          pi->dz=p0(i, particle_var::dz);
////          pi->dx=p0[i].dx; pi->dy=p0[i].dy; pi->dz=p0[i].dz;
//          pi->i =nn - range[face];
//          pi->ux=p0(i, particle_var::ux); 
//          pi->uy=p0(i, particle_var::uy); 
//          pi->uz=p0(i, particle_var::uz);
...
//        if( (nn>=0) & (nn<nb) ) {
//          particle_t particle;
//          particle.dx = p0(i, particle_var::dx);
//          particle.dy = p0(i, particle_var::dy);
//          particle.dz = p0(i, particle_var::dz);
//          particle.i  = sp->k_p_i_h(i);
//          particle.ux = p0(i, particle_var::ux);
//          particle.uy = p0(i, particle_var::uy);
//          particle.uz = p0(i, particle_var::uz);
//          particle.w  = p0(i, particle_var::w );
//          n_ci += pbc_interact[nn]( pbc_params[nn], sp, &particle, pm,
//                                    ci+n_ci, 1, face );
//          p0(i, particle_var::dx) = particle.dx;
//          p0(i, particle_var::dy) = particle.dy;
//          p0(i, particle_var::dz) = particle.dz;
//          sp->k_p_i_h(i) = particle.i;
//          p0(i, particle_var::ux) = particle.ux;
//          p0(i, particle_var::uy) = particle.uy;
//          p0(i, particle_var::uz) = particle.uz;
//          p0(i, particle_var::w ) = particle.w ;
//          goto backfill;
//        }
//
//        // Uh-oh: We fell through
...
////#       else
////        p0[i] = p0[np];
//        p0(i, particle_var::dx) = p0(np, particle_var::dx);
//        p0(i, particle_var::dy) = p0(np, particle_var::dy);
//        p0(i, particle_var::dz) = p0(np, particle_var::dz);
//        p0(i, particle_var::ux) = p0(np, particle_var::ux);
//        p0(i, particle_var::uy) = p0(np, particle_var::uy);
//        p0(i, particle_var::uz) = p0(np, particle_var::uz);
//        p0(i, particle_var::w ) = p0(np, particle_var::w );
//        sp->k_p_i_h(i) = sp->k_p_i_h(np);
////#       endif
//
//      }
//
