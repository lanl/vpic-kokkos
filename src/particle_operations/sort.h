...
//        bin_sort.sort(particles_i);
    ...
    static void tiled_sort(
            k_particles_soa_t part,   // Particle data
            const int32_t np,         // Total # of particles
            const int32_t num_bins,   // # of bins (cells)
    ...
    )
    {
        ...
        Kokkos::parallel_reduce("Get min/max bin post update", Kokkos::RangePolicy<>(0,part.i.extent(0)), min_max_functor(key_view), reducer);
        auto keys = key_view;
    ...
    )
    {
        ...
        Kokkos::parallel_reduce("Get max/min nppc", Kokkos::RangePolicy<>(0,num_bins), min_max_functor(bin_counter), nppc_reducer); 
