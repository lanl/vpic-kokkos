...
struct binary_collision_pipeline {
  ...
  k_particle_sortindex_t_ra spi_cellindex_ra, spj_cellindex_ra;
  ...
  )
  {
    ...
    if( spi->last_indexed != spi->g->step ) {
      sorter.sort( spi, false, true );
    }

    if( spj->last_indexed != spj->g->step ) {
      sorter.sort( spj, false, true );
    }
    ...
    shuffler.shuffle( spi, rp, false, true );
  ...
  )
  {
    ...
    Kokkos::parallel_for("binary_collision_pipeline::apply_model",
      ...
      KOKKOS_LAMBDA (const int ipart) {
        ...
        // We could eliminate cellindex in principle by doing
        // a brute force search for 0 <= idx < ni such that
        //      spi_sortindex_ra(i0 + idx) == ipart
        // but this seems not fast...

        const int   v    = spi_i(ipart);              // Voxel
        const int   idx  = spi_cellindex_ra(ipart);   // Intracell index
