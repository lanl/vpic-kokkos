...
void
vpic_simulation::hyb_dump_energies( const char *fname,
                                int append ) {
  double en_f[7], en_p;
  species_t *sp;
  FileIO fileIO;
  ...
  if( rank()==0 ) {
    ...
    if( status==fail ) ERROR(( "Could not open \"%s\".", fname ));
      ...
      if( append==0 ) {
        fileIO.print( "%% Layout\n%% step ex ey ez bx by bz 3/2nkTe" );
        LIST_FOR_EACH(sp,species_list)
          fileIO.print( " \"%s\"", sp->name );
        fileIO.print( "\n" );
        fileIO.print( "%% timestep = %e\n", grid->dt );
  ...
  field_array->kernel->energy_f( en_f, field_array );
  if( rank()==0 && status!=fail )
    fileIO.print( " %e %e %e %e %e %e %e",
                  en_f[0], en_f[1], en_f[2],
                  en_f[3], en_f[4], en_f[5],
		  en_f[6] );
  //ARI hyb
  LIST_FOR_EACH(sp,species_list) {
    en_p = hyb_energy_p( sp, interpolator_array );
    if( rank()==0 && status!=fail ) fileIO.print( " %e", en_p );
  }
...
vpic_simulation::hydro_dump( const char * speciesname,
  ...
  //ARI
  clear_hydro_array( hydro_array );
  hyb_accumulate_hydro_p( hydro_array, sp, interpolator_array );
  hyb_synchronize_hydro_array( hydro_array );
