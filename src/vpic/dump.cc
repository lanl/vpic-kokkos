...
vpic_simulation::dump_fluids( const char *fsp_name,
  ...
  WRITE_HEADER_V0( dump_type::hydro_dump,fsp->id,fsp->q/fsp->m,fileIO); // To-do: Needs changing?
...
# define fluid(x,y,z) fsp->fl[VOXEL(x,y,z, grid->nx,grid->ny,grid->nz)]
  ...
  if(dumpParams.format == band) {
    ...
    WRITE_HEADER_V0(dump_type::hydro_dump, fsp->id, fsp->q/fsp->m, fileIO);
...
#define DUMP_FIELD_TO_HDF5(DSET_NAME, ATTRIBUTE_NAME, ELEMENT_TYPE)                                           \
  ...
  sprintf(field_scratch, DUMP_DIR_FORMAT, "fields_hdf5");
  ...
  printf("local size   = %d  %d %d \n", field_local_size[0], field_local_size[1], field_local_size[2]);
...
void
vpic_simulation::dump_hydro_hdf5( const char *speciesname, const char *fbase, int ftag )
{
#define DUMP_HYDRO_TO_HDF5(DSET_NAME, ATTRIBUTE_NAME, ELEMENT_TYPE)                                           \
  ...
  printf("local size   = %d %d %d \n", hydro_local_size[0], hydro_local_size[1], hydro_local_size[2]);
  printf("global_offset = %d %d %d \n", global_offset[0], global_offset[1], global_offset[2]);
  printf("mpi-rank = %d, rank index = (%d, %d, %d) \n", rank(), _ix, _iy, _iz);
  fflush(stdout);
#endif
...
void
vpic_simulation::dump_particles_hdf5( const char *speciesname, const char *fbase, int ftag )
{
  char fname[256];
  char group_name[256];
