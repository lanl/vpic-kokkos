...
size_t
vpic_simulation::inject_particle_parallel( species_t * sp,
                                           double x,  double y,  double z,
                                           double ux, double uy, double uz,
                                           double w,  double age,
                                           int update_rhob ) {
  ...
  if( (x<x0) | (x>x1) | ( (x==x1) & (grid->bc[BOUNDARY(1,0,0)]>=0 ) ) ) return 0;
  if( (y<y0) | (y>y1) | ( (y==y1) & (grid->bc[BOUNDARY(0,1,0)]>=0 ) ) ) return 0;
  if( (z<z0) | (z>z1) | ( (z==z1) & (grid->bc[BOUNDARY(0,0,1)]>=0 ) ) ) return 0;

  // This node should inject the particle
    
  size_t p_index = Kokkos::atomic_fetch_inc(&(sp->np));
  if( p_index>=sp->max_np ) ERROR(( "No room to inject particle" ));
  ...
  sp->k_p_h(p_index, particle_var::dx) = static_cast<double>(x);
  sp->k_p_h(p_index, particle_var::dy) = static_cast<double>(y);
  sp->k_p_h(p_index, particle_var::dz) = static_cast<double>(z);
  sp->k_p_h(p_index, particle_var::ux) = static_cast<double>(ux);
  sp->k_p_h(p_index, particle_var::uy) = static_cast<double>(uy);
  sp->k_p_h(p_index, particle_var::uz) = static_cast<double>(uz);
  sp->k_p_h(p_index, particle_var::w)  = static_cast<double>(w);
  sp->k_p_i_h(p_index)                 = VOXEL(ix,iy,iz, nx,ny,nz);
...
vpic_simulation::inject_particle( species_t * sp,
  ...
  size_t p_index = sp->np++;
if(p_index >= sp->max_np)
printf("WARNING: injecting particle %zu past max_np %zu\n", p_index, sp->max_np);
  //k_particles_t& k_particles = sp->k_p_h;
  //k_particles_i_t& k_particles_i = sp->k_p_i_h;
